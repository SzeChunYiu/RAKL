#!/usr/bin/env bash
# Submit Paper-II ExperienceBenchmark v1.3_2 Phase-1 ORACLE @ 3B on LUNARC FS9.
# Bound to frozen protocol_subject_hash from research/paper2_experience_benchmark_v1_3_2/.
# learning_loop_mode=root_cause_v1; diagnostic_arm=ORACLE_PROCEDURE_UPPER_BOUND.
# Parent is floored v1.3_1 1.5B ORACLE (3476756); 3476742 remains INSTRUMENT_DEFECT.
...
REPO="$ROOT/repo"
PACKET_REL=research/paper2_experience_benchmark_v1_3_2
PROTOCOL_SUBJECT_HASH=c8afd4ca39ff2e4abce968a53bf52dab3854ebbb97ff0e73c3b151f1a42d27e8
CONTRACT_REL=research/paper2_experience_benchmark_v1_3_2/BATCH_CONTRACT_V1_3_2_ORACLE.json
...
RECEIPT_ROOT="$ROOT/receipts/experience_v1_3_2_oracle"
LOG_ROOT="$ROOT/logs/experience_v1_3_2_oracle"
RUN_ROOT="$ROOT/runs/experience_v1_3_2_oracle"
SBATCH_REL=experiments/paper2/lunarc/run_experience_benchmark_v1_3_2_oracle.sbatch
LEARNING_LOOP_MODE=root_cause_v1
DIAGNOSTIC_ARM=ORACLE_PROCEDURE_UPPER_BOUND
MODEL_SNAPSHOT="$ROOT/assets/paper2-model-qwen25-3b-v1/model/Qwen--Qwen2.5-3B-Instruct/aa8e72537993ba99e69dfaafa59ed015b17504d1"
...
witness=json.loads((repo/"research/paper2_experience_benchmark_v1_3_2/DIFFERENCE_WITNESS_V1_3_2.json").read_text(encoding="utf-8"))
...
if witness.get("scale_change_from_floored_1_5B_oracle") is not True: raise SystemExit("must record scale change from floored 1.5B ORACLE")
...
if witness.get("parent_scientific_verdict") != "MODEL_CAPABILITY_FLOOR_1_5B": raise SystemExit("parent must be floored 1.5B ORACLE")
if packet.get("primary_execution",{}).get("model_scale") != "Qwen2.5-3B-Instruct":
    raise SystemExit("packet model_scale must be 3B")
if packet.get("model",{}).get("model_id") != "Qwen/Qwen2.5-3B-Instruct":
    raise SystemExit("packet model_id must be Qwen2.5-3B-Instruct")
...
receipt={
  ...
  "verdict":"SUBMITTED_EXPERIENCE_BENCHMARK_V1_3_2_ORACLE",
  ...
  "experience_benchmark_3B_submitted":True,
  ...
  "claim_boundary":"Submission receipt only for #247 Phase-1 3B ORACLE after floored 1.5B. Not manuscript authority. Not experience-learning efficacy.",
}
