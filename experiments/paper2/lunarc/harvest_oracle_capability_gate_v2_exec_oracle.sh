#!/usr/bin/env bash
# Harvest ORACLE_CAPABILITY_GATE_V2_0_EXEC Phase-1 7B ORACLE artifacts for a completed SLURM job.
# Does not invoke the model. Does not reopen #138 or reinterpret prior floors as lift.
set -euo pipefail
...
RUN_DIR="$ROOT/runs/oracle_capability_gate_v2_exec/paper2-oracle-capability-gate-v2-exec-oracle-job-${JOB_ID}"
...
FORBIDDEN_JOBS="3476520 3476521 3476524 3476548 3476730 3476731 3476742 3476756 3476778 3476788"
for bad in $FORBIDDEN_JOBS; do
  if [[ "$JOB_ID" == "$bad" ]]; then
    echo "REFUSED: job $JOB_ID is not a V2_0_EXEC ORACLE harvest subject" >&2
    exit 42
  fi
done
...
if sorted({run["task_id"] for run in run_lines}) != ["T1","T2","T3","T4","T5"]:
    raise SystemExit("ORACLE harvest expects exactly T1–T5")
sn_hash=(run_dir/"states"/"Sn.hash").read_text(encoding="utf-8").strip()
receipt={
  "schema_version":"paper2-oracle-capability-gate-harvest-receipt-v1",
  "created_at_utc":datetime.datetime.now(datetime.timezone.utc).isoformat().replace("+00:00","Z"),
  "issue":379,
  "section":"PHASE1_ORACLE_7B_V2_EXEC",
  "verdict":"HARVESTED_ORACLE_CAPABILITY_GATE_V2_0_EXEC_AWAITING_VALIDATION",
  "slurm_job_id":job,
  ...
  "claim_boundary":"Harvest receipt only for #379 V2_0_EXEC ORACLE. Not CAPABLE_MODEL clearance until validation.",
