#!/usr/bin/env python3
"""Build the V4.3 task/seed receipt without reinterpreting any V4 output."""
...
def build(args: argparse.Namespace) -> None:
    ...
    if packet.get("seed_schedule") != [17]:
        raise RuntimeError("V4.3 seed schedule mismatch")
    ...
    if result.get("experiment_id") != PROTOCOL_ID or result.get("subject_sha") != packet.get("subject_sha"):
        raise RuntimeError("result does not match the V4.3 packet")
    if result.get("packet_sha256") != packet_sha:
        raise RuntimeError("result does not bind the exact V4.3 packet bytes")
    ...
    if pre.get("snapshot_canonical_sha256") != post.get("snapshot_canonical_sha256"):
        raise RuntimeError("snapshot changed across V4.3 inference")
    if pre.get("execution_checkout") != post.get("execution_checkout"):
        raise RuntimeError("execution checkout changed across V4.3 inference")
    if post.get("result_receipt_sha256") != _sha256(args.result):
        raise RuntimeError("post-inference attestation does not bind the V4.3 result")
    ...
    if result.get("execution_checkout") != expected_checkout:
        raise RuntimeError("result checkout does not match V4.3 attestations")

    records = result.get("records")
    if not isinstance(records, list) or len(records) != 2:
        raise RuntimeError("V4.3 requires exactly two arm records")
    if {row.get("condition") for row in records} != {"DIRECT_CORPUS", "RAKL_CONTEXT"}:
        raise RuntimeError("V4.3 arm set is incomplete or duplicated")
    ...
    for row in sorted(records, key=lambda item: str(item["condition"])):
        ...
        if not isinstance(score, dict) or not isinstance(score.get("parse_valid"), bool):
            raise RuntimeError("V4.3 score state missing")
        indexed.append(
            {
    ...
    if parse_valid_count != scorable_count:
        raise RuntimeError("V4.3 parse-valid and scorable counts diverge")

    receipt = {
        "schema_version": "paper2-pendulum-task-seed-receipt-v4.3",
        "receipt_type": "nonconfirmatory_v4_3_task_seed_execution_index",
        ...
        "records": indexed,
        "claim_boundary": (
            "One fresh non-confirmatory V4.3 task/seed unit only. This receipt does not "
