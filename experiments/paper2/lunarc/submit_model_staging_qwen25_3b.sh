#!/usr/bin/env bash
# Login-host operator wrapper for Qwen2.5-3B ExperienceBenchmark model-only staging.
...
REPO="$ROOT/repo"
CONTRACT_REL=research/paper2_experience_benchmark_v1_3_2/MODEL_STAGING_CONTRACT_3B.json
...
LOG_ROOT="$ROOT/logs/qwen25_3b_stage"
...
PROBE_JOB="$(sbatch --parsable \
  ...
  "$REPO/experiments/paper2/lunarc/network_probe_qwen25_3b.sbatch")"
PROBE_JOB="${PROBE_JOB%%;*}"
[[ "$PROBE_JOB" =~ ^[0-9]+$ ]]
...
STAGE_JOB="$(sbatch --parsable \
  ...
  "$REPO/experiments/paper2/lunarc/stage_model_assets_qwen25_3b.sbatch")"
STAGE_JOB="${STAGE_JOB%%;*}"
[[ "$STAGE_JOB" =~ ^[0-9]+$ ]]
...
receipt={
  ...
  "claim_boundary":"Submission receipt only for Qwen2.5-3B ExperienceBenchmark model-only staging; harvest required before ORACLE submit.",
}
