#!/usr/bin/env bash
# Login-host operator wrapper for Qwen2.5-7B ExperienceBenchmark model-only staging.
...
REPO="$ROOT/repo"
CONTRACT_REL=research/paper2_experience_benchmark_v1_3_3/MODEL_STAGING_CONTRACT_7B.json
...
RECEIPT_ROOT="$ROOT/receipts/qwen25_7b_stage"
LOG_ROOT="$ROOT/logs/qwen25_7b_stage"
...
PROBE_JOB="$(sbatch --parsable \
  ...
  "$REPO/experiments/paper2/lunarc/network_probe_qwen25_7b.sbatch")"
PROBE_JOB="${PROBE_JOB%%;*}"
[[ "$PROBE_JOB" =~ ^[0-9]+$ ]]
...
STAGE_JOB="$(sbatch --parsable \
  ...
  "$REPO/experiments/paper2/lunarc/stage_model_assets_qwen25_7b.sbatch")"
STAGE_JOB="${STAGE_JOB%%;*}"
[[ "$STAGE_JOB" =~ ^[0-9]+$ ]]
...
receipt={
  ...
  "claim_boundary":"Submission receipt only for Qwen2.5-7B ExperienceBenchmark model-only staging; harvest required before ORACLE submit.",
}
