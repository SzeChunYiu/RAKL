...
RUN_DIR="$ROOT/runs/v4_2/PENDULUM_SEALED_KNOWN_ANSWER_001-seed-17-job-${JOB_ID}"
ATTESTATION_DIR="$ROOT/receipts/v4_2/job-${JOB_ID}"
...
test -x "$PYTHON"
cd "$REPO"
"$PYTHON" experiments/paper2/lunarc/build_native_harvest_receipt_v4_2.py \
  --job-id "$JOB_ID" \
  --run-dir "$RUN_DIR" \
