#!/usr/bin/env python3
"""Build a fail-closed V4.4 native harvest receipt from exact LUNARC evidence."""
...
def build(args: argparse.Namespace) -> int:
    ...
    if packet.get("protocol_id") != PROTOCOL_ID or packet.get("output_normalization_policy_id") != POLICY_ID:
        failures.append("packet_v4_4_policy_or_protocol_mismatch")
    ...
    if packet.get("parent_v4_3_1_type_b_leak_known_before_freeze") is not True:
        failures.append("packet_parent_type_b_leak_disclosure_missing")
    if packet.get("positive_control_sensitivity_required") is not True:
        failures.append("packet_positive_control_requirement_missing")
    if packet.get("rakl_vs_direct_claim_from_leaked_parents_permitted") is not False:
        failures.append("packet_leaked_parent_reclaim_permitted")
    ...
    receipt = {
        "schema_version": "paper2-pendulum-native-harvest-receipt-v4.4",
        "created_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat().replace("+00:00", "Z"),
        "slurm_job_id": args.job_id,
        "verdict": "HARVEST_V4_4_TASK_SEED_PASS_NONCONFIRMATORY" if success else "HARVEST_V4_4_CANNOT_CHECK",
        "failures": failures,
        "scheduler_evidence": {
            "path": str(args.sacct),
        ...
        "claim_boundary": (
            "At most one fresh non-confirmatory V4.4 task/seed unit. A pass is receipt-chain "
