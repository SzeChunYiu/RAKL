#!/usr/bin/env python3
"""Build the governed repository-ingest receipt for a harvested V4.1 job.
...
task/seed, result, normalization, and immutable V4-negative lineage agree.
"""
...
def _verify_standalone_evidence(
    *, native: Path, run: Path, result: dict[str, Any], task_seed: dict[str, Any]
    ...
    for line in stdout_lines[1:]:
        ...
        marker = "/RAKL-paper2/"
        _require(marker in remote_path, "stdout evidence path is outside FS9 root")
        relative = Path(remote_path.split(marker, 1)[1])
        _require(
            not relative.is_absolute() and ".." not in relative.parts,
            "stdout evidence path escapes FS9 root",
        )
        local_path = native / relative
        _require(local_path.is_file(), f"stdout evidence file missing: {local_path}")
        _require(_sha(local_path) == digest, f"stdout evidence hash mismatch: {local_path}")
...
def build(*, job_id: str, created_at_utc: str, expected_execution_head: str) -> dict[str, Any]:
    ...
    run = native / f"runs/v4_1/{TASK_ID}-seed-17-job-{job_id}"
    receipt_root = native / "receipts/v4_1"
    bundle = V41 / f"native_bundles/PAPER2_V4_1_NATIVE_JOB_{job_id}.tar.gz"
    ...
    _require(harvest["verdict"] == "HARVEST_V4_1_TASK_SEED_PASS_NONCONFIRMATORY", "harvest did not pass")
    ...
    _require(scheduler["name"] == "p2-pend-v4-1", "scheduler name mismatch")
    ...
    receipt = {
        "schema_version": "paper2-v4-1-native-ingest-receipt-v1",
        "receipt_id": f"PAPER2_V4_1_NATIVE_JOB_{job_id}_INGEST_RECEIPT_20260811",
        "created_at_utc": created_at_utc,
        "verdict": "NATIVE_EXECUTION_CHAIN_PASS__ONE_ARM_SCORABLE_NO_EXACT_PASS__COMPARISON_NOT_ESTIMABLE",
        "object": "One native LUNARC execution of the adaptive non-confirmatory V4.1 pendulum task/seed replay under two prompt-materialization arms.",
        "qoi": "Did the exact receipt chain pass, which frozen V4.1 outputs were parse-valid/scorable, and did any arm achieve exact conceptual success?",
        "ingest_schema": {
        ...
        "native_execution": {
            ...
            "stderr_warning_lines": len((native / f"logs/v4_1/p2-pend-v4-1-{job_id}.err").read_text(encoding="utf-8").splitlines()),
        ...
        "typed_residual": {
            "residual_id": "PAPER2_V4_1_ONE_ARM_UNSCORABLE_AND_NO_EXACT_PASS",
