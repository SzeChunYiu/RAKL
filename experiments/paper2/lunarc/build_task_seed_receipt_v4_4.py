#!/usr/bin/env python3
"""Build the V4.4 task/seed receipt without reinterpreting any V4 output."""
...
PROTOCOL_ID = "PAPER2_PENDULUM_SEALED_KNOWN_ANSWER_V4_4"
...
def build(args: argparse.Namespace) -> None:
    ...
    if packet.get("protocol_id") != PROTOCOL_ID:
        raise RuntimeError("V4.4 protocol identity mismatch")
    if packet.get("chronology_class") != "ADAPTIVE_TYPE_B_LEAK_REPAIR_FRESH_ONLY_TO_V4_4_OUTPUTS":
        raise RuntimeError("V4.4 adaptive chronology class mismatch")
    if packet.get("parent_v4_3_1_type_b_leak_known_before_freeze") is not True:
        raise RuntimeError("V4.4 parent Type B leak disclosure missing")
    if packet.get("positive_control_sensitivity_required") is not True:
        raise RuntimeError("V4.4 positive-control requirement missing")
    ...
    if packet.get("evaluated_results_opened_before_freeze_scope") != "V4_4_OUTPUTS_ONLY_PARENT_LEAK_KNOWN":
        raise RuntimeError("V4.4 result-access scope mismatch")
    if packet.get("evaluated_task_seed_unit_count_before_freeze_scope") != "V4_4_OUTPUTS_ONLY_PARENT_LEAK_KNOWN":
        raise RuntimeError("V4.4 evaluated-unit scope mismatch")
    if packet.get("output_normalization_policy_id") != POLICY_ID:
        raise RuntimeError("V4.4 normalization policy identity mismatch")
    if packet.get("v4_4_outputs_opened_before_freeze") is not False:
        raise RuntimeError("V4.4 freeze chronology mismatch")
    if packet.get("threshold_or_score_change_permitted") is not False:
        raise RuntimeError("V4.4 exact-gate change prohibition missing")
    if packet.get("registered_task_id") != "PENDULUM_SEALED_KNOWN_ANSWER_001":
        raise RuntimeError("V4.4 task identity mismatch")
    if packet.get("seed_schedule") != [17]:
        raise RuntimeError("V4.4 seed schedule mismatch")
    ...
    if result.get("experiment_id") != PROTOCOL_ID or result.get("subject_sha") != packet.get("subject_sha"):
        raise RuntimeError("result does not match the V4.4 packet")
    if result.get("packet_sha256") != packet_sha:
        raise RuntimeError("result does not bind the exact V4.4 packet bytes")
    ...
    if pre.get("snapshot_canonical_sha256") != post.get("snapshot_canonical_sha256"):
        raise RuntimeError("snapshot changed across V4.4 inference")
    if pre.get("execution_checkout") != post.get("execution_checkout"):
        raise RuntimeError("execution checkout changed across V4.4 inference")
    if post.get("result_receipt_sha256") != _sha256(args.result):
        raise RuntimeError("post-inference attestation does not bind the V4.4 result")
    ...
    if result.get("execution_checkout") != expected_checkout:
        raise RuntimeError("result checkout does not match V4.4 attestations")

    records = result.get("records")
    if not isinstance(records, list) or len(records) != 2:
        raise RuntimeError("V4.4 requires exactly two arm records")
    if {row.get("condition") for row in records} != {"DIRECT_CORPUS", "RAKL_CONTEXT"}:
        raise RuntimeError("V4.4 arm set is incomplete or duplicated")
    ...
    for row in sorted(records, key=lambda item: str(item["condition"])):
        ...
        if not isinstance(score, dict) or not isinstance(score.get("parse_valid"), bool):
            raise RuntimeError("V4.4 score state missing")
        indexed.append(
            {
    ...
    if parse_valid_count != scorable_count:
        raise RuntimeError("V4.4 parse-valid and scorable counts diverge")

    receipt = {
        "schema_version": "paper2-pendulum-task-seed-receipt-v4.4",
        "receipt_type": "nonconfirmatory_v4_4_task_seed_execution_index",
        ...
        "records": indexed,
        "claim_boundary": (
            "One fresh non-confirmatory V4.4 task/seed unit only. This receipt does not "
