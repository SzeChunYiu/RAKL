#!/usr/bin/env python3
"""Build the governed repository-ingest receipt for a harvested V4.2 job.
...
def _verify_standalone_evidence(
    *,
    ...
    stderr = (native / f"logs/v4_2/p2-pend-v4-2-{job_id}.err").read_text(
        encoding="utf-8"
    )
    _require(stderr == warning * 2, "stderr warning bytes differ from admitted native log")

    stdout_lines = (
        native / f"logs/v4_2/p2-pend-v4-2-{job_id}.out"
...
def verify_ingest_receipt(receipt: dict[str, Any], *, root: Path = ROOT) -> None:
    ...
    prefix = f"research/paper2_microtrial_v4_2/native_job_{job_id}/"
    ...
    native_root = root / f"research/paper2_microtrial_v4_2/native_job_{job_id}"
...
def build(*, job_id: str, created_at_utc: str, expected_execution_head: str) -> dict[str, Any]:
    ...
    _require(harvest["verdict"] == "HARVEST_V4_2_TASK_SEED_PASS_NONCONFIRMATORY", "harvest did not pass")
    ...
    _require(scheduler["name"] == "p2-pend-v4-2", "scheduler name mismatch")
    ...
    receipt = {
        ...
        "native_execution": {
            ...
            "stderr_warning_lines": len((native / f"logs/v4_2/p2-pend-v4-2-{job_id}.err").read_text(encoding="utf-8").splitlines()),
