...
RUN_DIR="$ROOT/runs/v4_1/PENDULUM_SEALED_KNOWN_ANSWER_001-seed-17-job-${JOB_ID}"
ATTESTATION_DIR="$ROOT/receipts/v4_1/job-${JOB_ID}"
...
test -x "$PYTHON"
cd "$REPO"
"$PYTHON" experiments/paper2/lunarc/build_native_harvest_receipt_v4_1.py \
  --job-id "$JOB_ID" \
  --run-dir "$RUN_DIR" \
  --attestation-dir "$ATTESTATION_DIR" \
  --submission "$SUBMISSION_PATH" \
  --sacct "$SACCT_PATH" \
  --packet research/paper2_microtrial_v4_1/EXECUTION_PACKET_V4_1_20260811.json \
