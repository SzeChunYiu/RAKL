...
RUN_DIR="$ROOT/runs/v4_3/PENDULUM_SEALED_KNOWN_ANSWER_001-seed-17-job-${JOB_ID}"
ATTESTATION_DIR="$ROOT/receipts/v4_3/job-${JOB_ID}"
...
test -x "$PYTHON"
cd "$REPO"
"$PYTHON" experiments/paper2/lunarc/build_native_harvest_receipt_v4_3.py \
  --job-id "$JOB_ID" \
  --run-dir "$RUN_DIR" \
  --attestation-dir "$ATTESTATION_DIR" \
  --submission "$SUBMISSION_PATH" \
  --sacct "$SACCT_PATH" \
  --packet research/paper2_microtrial_v4_3/EXECUTION_PACKET_V4_3_20260811.json \
  --result-schema schemas/paper2-pendulum-microtrial-result.schema.json \
  --task-seed-schema schemas/paper2-pendulum-task-seed-receipt-v4-3.schema.json \
  --attestation-schema schemas/paper2-model-snapshot-attestation-v4.schema.json \
  --submission-schema schemas/paper2-pendulum-submission-receipt-v4-3.schema.json \
  --harvest-schema schemas/paper2-pendulum-native-harvest-receipt-v4-3.schema.json \
  --output "$HARVEST_PATH"
