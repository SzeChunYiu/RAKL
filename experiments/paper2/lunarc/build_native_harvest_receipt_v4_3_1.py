#!/usr/bin/env python3
"""Build a fail-closed V4.3.1 native harvest receipt from exact LUNARC evidence."""
...
def build(args: argparse.Namespace) -> int:
    ...
    if packet.get("protocol_id") != PROTOCOL_ID or packet.get("output_normalization_policy_id") != POLICY_ID:
        failures.append("packet_v4_3_1_policy_or_protocol_mismatch")
    ...
    receipt = {
        "schema_version": "paper2-pendulum-native-harvest-receipt-v4.3.1",
        "created_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat().replace("+00:00", "Z"),
        "slurm_job_id": args.job_id,
        "verdict": "HARVEST_V4_3_1_TASK_SEED_PASS_NONCONFIRMATORY" if success else "HARVEST_V4_3_1_CANNOT_CHECK",
        "failures": failures,
        "scheduler_evidence": {
            "path": str(args.sacct),
        ...
        "claim_boundary": (
            "At most one fresh non-confirmatory V4.3.1 task/seed unit. A pass is receipt-chain "
