#!/usr/bin/env bash
# Harvest Qwen2.5-7B model-only staging result.
set -euo pipefail
if [[ $# -ne 1 || ! "$1" =~ ^[0-9]+$ ]]; then
...
FINAL=/projects/hep/fs9/users/scyiu/RAKL-paper2/assets/paper2-model-qwen25-7b-v1
...
receipt={
  ...
  "claim_boundary":"Harvest only for Qwen2.5-7B ExperienceBenchmark model staging; no ORACLE score.",
}
