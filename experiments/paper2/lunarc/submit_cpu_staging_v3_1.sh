...
contract="$repo/research/paper2_microtrial_v3/CPU_STAGING_CONTRACT_V3_1.json"
mapfile -t associations < <(sacctmgr -nP show assoc user="$USER" format=Account,Partition | awk -F'|' 'NF>=2 && $1!="" && $2!="" {print $1 ":" $2}')
argv=(python3 -m rakl.paper2_cpu_staging_v3_1 submit --contract "$contract" --repo "$repo" --expected-repo-sha "$expected" --bootstrap-receipt "$bootstrap_receipt" --account "$account" --partition "$partition" --receipt-output "$receipt")
