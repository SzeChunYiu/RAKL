#!/usr/bin/env python3
"""Build the governed repository-ingest receipt for a harvested V4.3 job.
...
task/seed, result, normalization, and immutable V4.2 capability-limit parent agree.
"""
...
SCHEMA = ROOT / "schemas/paper2-v4-3-native-ingest-receipt.schema.json"
...
PACKET_HEAD_SHA = "57e9bc041eccc091fdf86a20a7a8c78dff037727"
OUTCOME_REASON = (
    "DIRECT_CORPUS remains parse-invalid/null under the frozen exact V4.1 "
...
def _verify_standalone_evidence(
    *,
    ...
    stderr = (native / f"logs/v4_3/p2-pend-v4-3-{job_id}.err").read_text(
        encoding="utf-8"
    )
    _require(stderr == warning * 2, "stderr warning bytes differ from admitted native log")

    stdout_lines = (
        native / f"logs/v4_3/p2-pend-v4-3-{job_id}.out"
...
def verify_ingest_receipt(receipt: dict[str, Any], *, root: Path = ROOT) -> None:
    ...
    prefix = f"research/paper2_microtrial_v4_3/native_job_{job_id}/"
    ...
    native_root = root / f"research/paper2_microtrial_v4_3/native_job_{job_id}"
...
def build(*, job_id: str, created_at_utc: str, expected_execution_head: str) -> dict[str, Any]:
    native = V43 / f"native_job_{job_id}"
    run = native / f"runs/v4_3/{TASK_ID}-seed-17-job-{job_id}"
    receipt_root = native / "receipts/v4_3"
    bundle = V43 / f"native_bundles/PAPER2_V4_3_NATIVE_JOB_{job_id}.tar.gz"
    ...
    component_schemas = {
        "submission": ROOT / "schemas/paper2-pendulum-submission-receipt-v4-3.schema.json",
        ...
        "harvest": ROOT / "schemas/paper2-pendulum-native-harvest-receipt-v4-3.schema.json",
        "result": ROOT / "schemas/paper2-pendulum-microtrial-result.schema.json",
        "task_seed": ROOT / "schemas/paper2-pendulum-task-seed-receipt-v4-3.schema.json",
    }
    ...
    _require(harvest["verdict"] == "HARVEST_V4_3_TASK_SEED_PASS_NONCONFIRMATORY", "harvest did not pass")
    ...
    _require(checkout == result["execution_checkout"] == task_seed["execution_checkout"] == manifest["execution_checkout"], "execution checkout lineage mismatch")
    ...
    _require(pre["snapshot_canonical_sha256"] == post["snapshot_canonical_sha256"] == task_seed["snapshot_attestations"]["snapshot_canonical_sha256"], "snapshot identity mismatch")
    ...
    _require(scheduler["name"] == "p2-pend-v4-3", "scheduler name mismatch")
    ...
    v42_ingest = V42 / "PAPER2_V4_2_NATIVE_JOB_3476540_INGEST_RECEIPT_20260811.json"
    v4_negative = _load(v42_ingest)
    _require(
        manifest["bound_artifact_sha256"]["v4_2_capability_limit_parent"] == _sha(v42_ingest),
        "V4.2 capability-limit parent binding mismatch",
    )
    _require(v4_negative["task_seed_outcome"]["exact_conceptual_pass_arm_count"] == 0, "V4.2 exact-pass history changed")
    _require(v4_negative["task_seed_outcome"]["score_comparison_permitted"] is False, "V4.2 comparison enabled")
    ...
    receipt = {
        "schema_version": "paper2-v4-3-native-ingest-receipt-v1",
        "receipt_id": f"PAPER2_V4_3_NATIVE_JOB_{job_id}_INGEST_RECEIPT_20260811",
        "created_at_utc": created_at_utc,
        "verdict": "NATIVE_EXECUTION_CHAIN_PASS__ONE_ARM_SCORABLE_NO_EXACT_PASS__1_5B_NO_EXACT_PASS",
        "object": "One native LUNARC execution of the adaptive non-confirmatory V4.3 pendulum task/seed replay under two prompt-materialization arms on Qwen2.5-1.5B-Instruct.",
        "qoi": "Did the exact receipt chain pass, which frozen V4.3 outputs were parse-valid/scorable, and did any arm achieve exact conceptual success under the unchanged V4.1 gate?",
        "ingest_schema": {
        ...
        "native_execution": {
            ...
            "stderr_warning_lines": len((native / f"logs/v4_3/p2-pend-v4-3-{job_id}.err").read_text(encoding="utf-8").splitlines()),
        ...
        "v4_negative_parent": {
            "path": v42_ingest.relative_to(ROOT).as_posix(),
            "sha256": _sha(v42_ingest),
        ...
        "typed_residual": {
            "residual_id": "PAPER2_V4_3_DIRECT_PARSE_NULL_AND_1_5B_NO_EXACT_PASS",
            "root_cause_ladder": ["R1_SCHEMA_PARSER_TRANSFORMATION", "R7_PROJECTION_FUNCTIONAL_FORM"],
            "observed_signature": "DIRECT_CORPUS remains parse-invalid (registered answer schema field mismatch); RAKL_CONTEXT is the sole scorable arm at conceptual 2/5 with exact_conceptual_pass=false under the unchanged V4.1 exact gate on Qwen2.5-1.5B-Instruct.",
            "null_or_competing_explanations": [
                "Prompt architecture may affect serialization independently of scientific content.",
                "The 1.5B model still fails the exact conceptual gate on the sole scorable arm (2/5); larger size alone did not mint exact passes.",
            ...
            "next_discriminator": "SIBLING_OWNS_DIRECT_PARSE_ROOT_CAUSE: do not soften exact_conceptual_pass; do not resubmit from this ingest fiber. Preserve DIRECT parse_valid=false as negative history. Paper-eligible promotional metrics remain BLOCKED (sealed microtrial authority only).",
        },
        "quantitative_figure_generated": False,
        "claim_boundary": "Adaptive non-confirmatory engineering evidence only. V4.2 0.5B remains both-arms-scorable with exact_conceptual_pass_arm_count=0; V4.3 1.5B job 3476566 is one-arm-scorable with exact_conceptual_pass_arm_count=0 (RAKL_CONTEXT 2/5). No invented passes, no gate softening, no arm win/loss, paired effect, promotional metric, #138 experience section B authority, general superiority, independent review, or peer-review claim is permitted.",
    }
