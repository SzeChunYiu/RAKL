#!/usr/bin/env python3
"""Build the governed repository-ingest receipt for a harvested V4.4 job.
...
V431 = ROOT / "research/paper2_microtrial_v4_4"
SCHEMA = ROOT / "schemas/paper2-v4-4-native-ingest-receipt.schema.json"
...
def _verify_standalone_evidence(
    *,
    ...
    stderr = (native / f"logs/v4_4/p2-pend-v44-{job_id}.err").read_text(
        encoding="utf-8"
    )
    _require(stderr == warning * 2, "stderr warning bytes differ from admitted native log")

    stdout_lines = (
        native / f"logs/v4_4/p2-pend-v44-{job_id}.out"
...
def verify_ingest_receipt(receipt: dict[str, Any], *, root: Path = ROOT) -> None:
    ...
    prefix = f"research/paper2_microtrial_v4_4/native_job_{job_id}/"
    ...
    native_root = root / f"research/paper2_microtrial_v4_4/native_job_{job_id}"
...
def build(*, job_id: str, created_at_utc: str, expected_execution_head: str) -> dict[str, Any]:
    ...
    run = native / f"runs/v4_4/{TASK_ID}-seed-17-job-{job_id}"
    receipt_root = native / "receipts/v4_4"
    bundle = V431 / f"native_bundles/PAPER2_V4_4_NATIVE_JOB_{job_id}.tar.gz"
    ...
    component_schemas = {
        "submission": ROOT / "schemas/paper2-pendulum-submission-receipt-v4-4.schema.json",
        ...
        "harvest": ROOT / "schemas/paper2-pendulum-native-harvest-receipt-v4-4.schema.json",
        "result": ROOT / "schemas/paper2-pendulum-microtrial-result.schema.json",
        "task_seed": ROOT / "schemas/paper2-pendulum-task-seed-receipt-v4-4.schema.json",
    }
    ...
    _require(harvest["verdict"] == "HARVEST_V4_4_TASK_SEED_PASS_NONCONFIRMATORY", "harvest did not pass")
    ...
    _require(scheduler["name"] == "p2-pend-v44", "scheduler name mismatch")
    ...
    _require(isinstance(direct.get("score"), dict), "DIRECT_CORPUS score record missing")
    _require(isinstance(rakl.get("score"), dict), "RAKL_CONTEXT score record missing")
    # Do not bake expected conceptual scores: V4.4 is a fresh leak-repair run.
    _verify_standalone_evidence(
    ...
    disposition = (
        ROOT
        / "research/paper2_microtrial_v4_3_1/TYPE_B_LEAK_DISPOSITION_283.json"
    )
    disposition_payload = _load(disposition)
    _require(
        manifest["bound_artifact_sha256"]["parent_v4_3_1_type_b_disposition"]
        == _sha(disposition),
        "V4.3.1 Type B disposition parent binding mismatch",
    )
    _require(
        disposition_payload.get("verdict") == "NOT_INFORMATIVE",
        "V4.3.1 Type B disposition verdict changed",
    )
    _require(
        disposition_payload.get("status") == "SEALED_KNOWN_TYPE_B_DEGENERATE",
    ...
    receipt = {
        "schema_version": "paper2-v4-4-native-ingest-receipt-v1",
        "receipt_id": f"PAPER2_V4_4_NATIVE_JOB_{job_id}_INGEST_RECEIPT_20260811",
        "created_at_utc": created_at_utc,
        "verdict": "NATIVE_EXECUTION_CHAIN_PASS__BOTH_ARMS_SCORABLE_NO_EXACT_PASS__SERIALIZATION_REPAIR_STILL_ZERO_EXACT",
        "object": (
            "One native LUNARC execution of the adaptive non-confirmatory V4.4 pendulum "
        ...
        "qoi": (
            "Did the exact receipt chain pass, which frozen V4.4 outputs were "
        ...
        "native_execution": {
            ...
            "stderr_warning_lines": len(
                (native / f"logs/v4_4/p2-pend-v44-{job_id}.err")
