#!/usr/bin/env bash
# Harvest ExperienceBenchmark v1.3_1 Phase-1 1.5B ORACLE artifacts for a completed SLURM job.
# Does not invoke the model. Does not reopen #138 or reinterpret 3476548/3476730 as lift.
set -euo pipefail
...
RUN_DIR="$ROOT/runs/experience_v1_3_1_oracle/paper2-experience-benchmark-v1_3_1-oracle-job-${JOB_ID}"
...
receipt={
  ...
  "verdict":"HARVESTED_EXPERIENCE_BENCHMARK_V1_3_1_ORACLE_AWAITING_VALIDATION",
  "slurm_job_id":job,
  ...
  "claim_boundary":"Harvest receipt only for #247 Phase-1 1.5B ORACLE. validate + protected attestations still required before manuscript ingest.",
