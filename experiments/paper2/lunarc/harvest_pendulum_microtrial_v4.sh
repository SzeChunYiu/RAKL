#!/usr/bin/env bash
set -euo pipefail

if [[ $# -ne 1 || ! "$1" =~ ^[0-9]+$ ]]; then
  echo "usage: $0 <slurm-job-id>" >&2
  exit 64
fi

JOB_ID="$1"
ROOT=/projects/hep/fs9/users/scyiu/RAKL-paper2
RUN_DIR="$ROOT/runs/v4/PENDULUM_SEALED_KNOWN_ANSWER_001-seed-17-job-${JOB_ID}"
ATTESTATION_DIR="$ROOT/receipts/v4/job-${JOB_ID}"
RECEIPT_ROOT="$ROOT/receipts/v4"
SACCT_PATH="$RECEIPT_ROOT/sacct-${JOB_ID}.json"
HARVEST_PATH="$RECEIPT_ROOT/harvest-${JOB_ID}.json"
REPO="$ROOT/repo"
PYTHON="$ROOT/assets/paper2-cpu-v3-2/runtime/python/bin/python3.11"
mkdir -p "$RECEIPT_ROOT"
sacct -j "$JOB_ID" --json > "$SACCT_PATH"

test -x "$PYTHON"
cd "$REPO"
"$PYTHON" experiments/paper2/lunarc/build_native_harvest_receipt_v4.py \
  --job-id "$JOB_ID" \
  --run-dir "$RUN_DIR" \
  --attestation-dir "$ATTESTATION_DIR" \
  --sacct "$SACCT_PATH" \
  --result-schema schemas/paper2-pendulum-microtrial-result.schema.json \
  --task-seed-schema schemas/paper2-pendulum-task-seed-receipt-v4.schema.json \
  --attestation-schema schemas/paper2-model-snapshot-attestation-v4.schema.json \
  --harvest-schema schemas/paper2-pendulum-native-harvest-receipt-v4.schema.json \
  --output "$HARVEST_PATH"
