#!/usr/bin/env bash
# Harvest ExperienceBenchmark v1.3_3 Phase-1 7B ORACLE artifacts for a completed SLURM job.
...
RUN_DIR="$ROOT/runs/experience_v1_3_3_oracle/paper2-experience-benchmark-v1_3_3-oracle-job-${JOB_ID}"
...
if [[ "$JOB_ID" == "3476520" || "$JOB_ID" == "3476521" || "$JOB_ID" == "3476524" || "$JOB_ID" == "3476548" || "$JOB_ID" == "3476756" || "$JOB_ID" == "3476742" || "$JOB_ID" == "3476778" ]]; then
  echo "REFUSED: job $JOB_ID is not a v1.3_3 ORACLE harvest subject" >&2
  exit 42
fi
...
receipt={
  ...
  "verdict":"HARVESTED_EXPERIENCE_BENCHMARK_V1_3_3_ORACLE_AWAITING_VALIDATION",
  "slurm_job_id":job,
  ...
  "claim_boundary":"Harvest receipt only for #372 Phase-1 7B ORACLE. validate + protected attestations still required before manuscript ingest.",
