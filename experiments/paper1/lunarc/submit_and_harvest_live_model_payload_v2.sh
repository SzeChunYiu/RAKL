...
usage() {
  cat <<'EOF'
Usage:
  submit_and_harvest_live_model_payload_v2.sh preflight <SUBJECT_SHA> [REPO_ROOT]
  submit_and_harvest_live_model_payload_v2.sh submit    <SUBJECT_SHA> [REPO_ROOT]
  submit_and_harvest_live_model_payload_v2.sh harvest   <JOB_ID> <SUBJECT_SHA> [REPO_ROOT]
EOF
}
...
TRANSPORT_REL="$PACKET_REL/EXECUTION_TRANSPORT_V2.json"
...
for group in ('frozen_experiment_blobs', 'frozen_production_subject_blobs'):
    for path, expected in transport[group].items():
        actual = subprocess.check_output(['git','hash-object',path], text=True).strip()
        if actual != expected:
            raise SystemExit(f'{group.upper()}_MISMATCH:{path}:{actual}:{expected}')
path=Path('experiments/paper1/build_live_model_payload_panel_v1.py')
spec=importlib.util.spec_from_file_location('p1panel_transport_v2', path)
...
assert fixed['scientific_settings_changed_from_v1'] is False
assert transport['grants_scientific_authority'] is False
print('P1 frozen experiment + production subject: PASS')
PY
  ...
  python -m pytest -q \
    tests/test_driver_model_authority_ingress.py \
    tests/test_driver_authority_parser_chokepoint.py \
    tests/test_authority_chokepoint.py \
    tests/test_raw_agent_authority_gateway.py \
    tests/test_v3_scientific_authority_noninterference.py >/dev/null
  rm -rf "$TMP"
  trap - RETURN
}
...
receipt={
  'schema_version':'paper1-live-model-payload-submission-receipt-v2',
  'submitted_at':datetime.now(timezone.utc).isoformat(),
  'subject_sha':subject,
  'slurm_job_id':str(job_id),
  'model_path':model_path,
  'transport_sha256':hashlib.sha256(transport_path.read_bytes()).hexdigest(),
  'frozen_experiment_blobs':transport['frozen_experiment_blobs'],
  'frozen_production_subject_blobs':transport['frozen_production_subject_blobs'],
...
if [ "$MODE" = preflight ]; then
  ...
  echo "P1_EXECUTION_V2_PREFLIGHT_PASS subject=$SUBJECT_SHA"
  exit 0
fi
...
if [ "$MODE" = submit ]; then
  ...
  echo "P1_SUBMITTED_V2 job_id=$JOB_ID subject=$SUBJECT_SHA"
  exit 0
fi
...
python - "$SUBMISSION" "$SUBJECT_SHA" "$JOB_ID" "$EXACT_MODEL_PATH" "$REPO_ROOT/$TRANSPORT_REL" <<'PY'
import json, sys
from pathlib import Path
r=json.loads(Path(sys.argv[1]).read_text()); transport=json.loads(Path(sys.argv[5]).read_text())
assert r['schema_version']=='paper1-live-model-payload-submission-receipt-v2'
assert r['subject_sha']==sys.argv[2]
assert str(r['slurm_job_id'])==str(sys.argv[3])
assert r['model_path']==sys.argv[4]
assert r['frozen_production_subject_blobs']==transport['frozen_production_subject_blobs']
...
if submission.get('frozen_production_subject_blobs')!=transport['frozen_production_subject_blobs']: raise SystemExit('production_subject_submission_mismatch')
...
receipt={
 'schema_version':'paper1-live-model-payload-harvest-receipt-v2',
...
echo "P1_HARVESTED_V2 job_id=$JOB_ID subject=$SUBJECT_SHA receipt=$HARVEST"
