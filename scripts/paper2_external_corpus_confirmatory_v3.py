"""Paper II external-corpus confirmatory runner (ARN v3 epoch).

Implements research/paper2_external_corpus_v1/PROTOCOL_V3_REDUCER.json exactly:
...
Uses narrative_reducer_v3.py (inherits v2 extraction) and instance_paired_mapping.py
for strict instance-paired correspondence scoring.

Key difference from v2: scoring is instance-paired; every term consumes (q_i, c_j) pairs.
No marginal components computable from one narrative alone.

Usage (laptop billy):
    PYTHONPATH=src:. .venv/bin/python scripts/paper2_external_corpus_confirmatory_v3.py \
        --csv <ARN csv> --out research/paper2_external_corpus_v1/results_v3_reducer
"""
...
from rakl.narrative_reducer_v3 import reduce_narrative_v3, content_tokens
from rakl.reduction_validation import AdmissionVerdict, ReducerProfile, admit_reducer
from rakl.instance_paired_mapping import instance_paired_match_decision, CorrespondenceResult
from rakl.structure_space import MatchVerdict

PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL_V3_REDUCER.json"
PARENT_PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL.json"
PARENT_V2_PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL_V2_REDUCER.json"
SPLIT_SALT = "20260814"
PARENT_RESULT = "research/paper2_external_corpus_v1/results/RESULT.json"
PARENT_V2_RESULT = "research/paper2_external_corpus_v1/results_v2_reducer/RESULT.json"
...
# --- v3 witness decision (instance-paired mapping + abstention) -------------


def witness_decision_v3(
    query_text: str,
    candidate_text: str,
    theta_w: float,
) -> tuple[str, tuple, CorrespondenceResult | None]:
    """v3 witness decision with instance-paired mapping and abstention.

    Returns (decision, signature, correspondence_result).
    """
    query_reduced = reduce_narrative_v3(query_text)
    candidate_reduced = reduce_narrative_v3(candidate_text)
    ...
    # Instance-paired mapping decision
    correspondence_result = instance_paired_match_decision(query_reduced, candidate_reduced, theta_w)

    return correspondence_result.decision, signature, correspondence_result


# For compatibility, alias to witness_decision
witness_decision = witness_decision_v3
...
def run(csv_path: Path, out_dir: Path) -> dict:
    ...
    result: dict = {
        "schema_version": "paper2-external-corpus-result-v3",
        "protocol": PROTOCOL,
    ...
    profile = ReducerProfile(
        reducer_id="narrative_reducer_v3",
    ...
    def witness_dev_score(theta: float) -> float:
        decided = [
            (witness_decision_v3(p.query_text, p.candidate_text, theta)[0], p.gold) for p in dev
        ]
    ...
    witness_out = [witness_decision_v3(p.query_text, p.candidate_text, theta_w) for p in confirm]
    ...
    for pair, signature in zip(confirm, witness_sigs):
        ...
        decision_s, signature_s, _ = witness_decision_v3(scrambled_query, scrambled_candidate, theta_w)
    ...
    for wr in witness_results:
        if wr and wr.abstention_reason:
            ...
            elif "zero_correspondence" in wr.abstention_reason:
                abstention_breakdown["zero_correspondence"] += 1
    ...
    try:
        ...
        # Load parent v2 result
        v2_path = Path(PARENT_V2_RESULT)
        if v2_path.exists():
            v2_data = json.loads(v2_path.read_text())
            parent_comparison["parent_v2"] = {
                "witness_exact": v2_data.get("gates", {}).get("arm_confirm_exact", {}).get("witness"),
                "band_exact": v2_data.get("gates", {}).get("arm_confirm_exact", {}).get("band"),
                "advantage": v2_data.get("gates", {}).get("G1_advantage", {}).get("advantage"),
                "ci": v2_data.get("gates", {}).get("G1_advantage", {}).get("ci"),
                "B3_shuffled_gold_advantage": v2_data.get("battery", {}).get("B3_shuffled_gold", {}).get("advantage"),
                "B3_shuffled_gold_ci": v2_data.get("battery", {}).get("B3_shuffled_gold", {}).get("ci"),
            }
    except Exception:
