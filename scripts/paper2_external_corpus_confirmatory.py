"""Paper II external-corpus confirmatory runner (ARN epoch).

Implements research/paper2_external_corpus_v1/PROTOCOL.json exactly:

    acquisition receipt -> schema binding -> reducer admission ->
    falsifiability battery -> confirmatory gates -> terminal

Fail-closed at every stage; every terminal in the protocol is a first-class
outcome. Arms never receive the gold field. Proposal-only; grants no authority.

Usage (laptop billy):
    PYTHONPATH=src:. .venv/bin/python scripts/paper2_external_corpus_confirmatory.py \
        --csv <ARN csv> --out research/paper2_external_corpus_v1/results
"""
...
from rakl.narrative_reducer import content_tokens, reduce_narrative
...
PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL.json"
SPLIT_SALT = "20260814"
...
# --- arms (gold-isolated: signatures take texts/band only) ---------------------


def witness_decision(query_text: str, candidate_text: str, theta_w: float) -> tuple[str, tuple]:
    query_reduced = reduce_narrative(query_text)
    candidate_reduced = reduce_narrative(candidate_text)
    signature = (tuple(sorted(query_reduced.roles)), tuple(sorted(candidate_reduced.roles)))
    if (
        len(query_reduced.roles) < 2
        or not query_reduced.relations
        or len(candidate_reduced.roles) < 2
        or not candidate_reduced.relations
    ):
        return "CANNOT_CHECK", signature
    problem = ProblemStructure(
        problem_id="pair",
        qoi="analogical_support",
        required_roles=query_reduced.roles,
        required_relations=query_reduced.relations,
    )
    space = StructureSpace(space_id="pair")
    space.accumulate(candidate_reduced)
    (result,) = match(space, problem)
    if result.verdict is not MatchVerdict.LICENSED:
        return "REJECT", signature
    denominator = len(problem.required_roles) + 2 * len(problem.required_relations)
    ratio = (len(result.covered_roles) + 2 * len(result.covered_relations)) / denominator
    return ("ACCEPT" if ratio >= theta_w else "REJECT"), signature
...
def run(csv_path: Path, out_dir: Path) -> dict:
    ...
    result: dict = {
        "schema_version": "paper2-external-corpus-result-v1",
        "protocol": PROTOCOL,
    ...
    admission = admit_reducer(profile, reduce_narrative, sample_sources, seed=SEED_SCRAMBLE)
    result["admission"] = {
    ...
    def witness_dev_score(theta: float) -> float:
        decided = [
            (witness_decision(p.query_text, p.candidate_text, theta)[0], p.gold) for p in dev
        ]
    ...
    witness_out = [witness_decision(p.query_text, p.candidate_text, theta_w) for p in confirm]
    witness_conf = [d for d, _ in witness_out]
    witness_sigs = [s for _, s in witness_out]
    controls_conf = {
    ...
    for pair, signature in zip(confirm, witness_sigs):
        ...
        decision_s, signature_s = witness_decision(scrambled_query, scrambled_candidate, theta_w)
    ...
    result["gates"] = {
        ...
        "G3_abstention": {"cannot_check_rate": cc_rate, "dominant": cc_rate > 0.50},
        "quadrants": {
