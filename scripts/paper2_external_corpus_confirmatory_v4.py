"""Paper II external-corpus confirmatory runner (ARN v4 epoch).

Implements research/paper2_external_corpus_v1/PROTOCOL_V4_REDUCER.json exactly:
    acquisition receipt -> schema binding -> reducer admission ->
    B3 FIRST (hard falsifier ordering) -> remaining battery -> confirmatory gates -> terminal

Uses narrative_reducer_v4.py (inherits v3 extraction) and relational_correspondence.py
for second-order relation-triple alignment (M1 Progressive Relational Abstraction parent).

Key difference from v3: scoring aligns RELATION TRIPLES (source_role_type, relation_type, target_role_type),
capturing second-order structural correspondence beyond flat typed-role/relation features.

Usage (laptop billy):
    PYTHONPATH=src:. .venv/bin/python scripts/paper2_external_corpus_confirmatory_v4.py \
        --csv <ARN csv> --out research/paper2_external_corpus_v1/results_v4_reducer
"""
...
from rakl.narrative_reducer_v4 import reduce_narrative_v4, content_tokens
from rakl.reduction_validation import AdmissionVerdict, ReducerProfile, admit_reducer
from rakl.relational_correspondence import relational_match_decision, CorrespondenceResult
from rakl.structure_space import MatchVerdict

PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL_V4_REDUCER.json"
PARENT_PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL.json"
PARENT_V3_PROTOCOL = "research/paper2_external_corpus_v1/PROTOCOL_V3_REDUCER.json"
SPLIT_SALT = "20260814"
PARENT_RESULT = "research/paper2_external_corpus_v1/results/RESULT.json"
PARENT_V3_RESULT = "research/paper2_external_corpus_v1/results_v3_reducer/RESULT.json"
...
# --- v4 witness decision (relational-correspondence mapping + abstention) --


def witness_decision_v4(
    query_text: str,
    candidate_text: str,
    theta_w: float,
) -> tuple[str, tuple, CorrespondenceResult | None]:
    """v4 witness decision with relational-correspondence mapping and abstention.

    Returns (decision, signature, correspondence_result).
    """
    query_reduced = reduce_narrative_v4(query_text)
    candidate_reduced = reduce_narrative_v4(candidate_text)
    ...
    # Relational-correspondence mapping decision
    correspondence_result = relational_match_decision(query_reduced, candidate_reduced, theta_w)

    return correspondence_result.decision, signature, correspondence_result


# For compatibility, alias to witness_decision
witness_decision = witness_decision_v4
...
def run(csv_path: Path, out_dir: Path) -> dict:
    ...
    result: dict = {
        "schema_version": "paper2-external-corpus-result-v4",
        "protocol": PROTOCOL,
    ...
    profile = ReducerProfile(
        reducer_id="narrative_reducer_v4",
        author="RAKL programme (same-context; LLM-assisted)",
        external_label_author="Sourati, Ilievski, Sommerauer, Jiang (ARN, TACL 2024)",
    )
    admission = admit_reducer(profile, reduce_narrative_v4, sample_sources, seed=SEED_SCRAMBLE)
    result["admission"] = {
    ...
    def witness_dev_score(theta: float) -> float:
        decided = [
            (witness_decision_v4(p.query_text, p.candidate_text, theta)[0], p.gold) for p in dev
        ]
    ...
    witness_out = [witness_decision_v4(p.query_text, p.candidate_text, theta_w) for p in confirm]
    ...
    for pair, signature in zip(confirm, witness_sigs):
        ...
        decision_s, signature_s, _ = witness_decision_v4(scrambled_query, scrambled_candidate, theta_w)
    ...
    # Abstention breakdown (v4 telemetry)
    abstention_breakdown = {
        "insufficient_extraction_evidence_relations": 0,
        "degenerate_relation_coverage": 0,
        "zero_triple_correspondence": 0,
        "total": cc_rate,
    }
    for wr in witness_results:
        if wr and wr.abstention_reason:
            if "insufficient_extraction_evidence" in wr.abstention_reason:
                abstention_breakdown["insufficient_extraction_evidence_relations"] += 1
            elif "degenerate_relation_coverage" in wr.abstention_reason:
                abstention_breakdown["degenerate_relation_coverage"] += 1
            elif "zero_triple_correspondence" in wr.abstention_reason:
                abstention_breakdown["zero_triple_correspondence"] += 1
    ...
    parent_comparison = {}
    try:
        # Load parent v1 result
        parent_path = Path(PARENT_RESULT)
        if parent_path.exists():
            parent_data = json.loads(parent_path.read_text())
            parent_comparison["parent_v1"] = {
                "witness_exact": parent_data.get("gates", {}).get("arm_confirm_exact", {}).get("witness"),
                "band_exact": parent_data.get("gates", {}).get("arm_confirm_exact", {}).get("band"),
                "advantage": parent_data.get("gates", {}).get("G1_advantage", {}).get("advantage"),
                "ci": parent_data.get("gates", {}).get("G1_advantage", {}).get("ci"),
            }
        # Load parent v3 result
        v3_path = Path(PARENT_V3_RESULT)
        if v3_path.exists():
            v3_data = json.loads(v3_path.read_text())
            parent_comparison["parent_v3"] = {
                "witness_exact": v3_data.get("gates", {}).get("arm_confirm_exact", {}).get("witness"),
                "band_exact": v3_data.get("gates", {}).get("arm_confirm_exact", {}).get("band"),
                "advantage": v3_data.get("gates", {}).get("G1_advantage", {}).get("advantage"),
                "ci": v3_data.get("gates", {}).get("G1_advantage", {}).get("ci"),
                "B3_shuffled_gold_advantage": v3_data.get("battery", {}).get("B3_shuffled_gold", {}).get("advantage"),
                "B3_shuffled_gold_ci": v3_data.get("battery", {}).get("B3_shuffled_gold", {}).get("ci"),
            }
    except Exception:
        parent_comparison = {"note": "Could not load parent results"}
