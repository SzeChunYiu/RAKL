...
def test_frozen_benchmark_precedes_implementation_and_contains_hostile_worlds():
    path = Path(__file__).parents[1] / "research" / "SELF_RAKL_RESEARCH_012_FROZEN_BENCHMARK.json"
    data = json.loads(path.read_text())
    assert data["status"] == "FROZEN_BEFORE_IMPLEMENTATION"
    assert data["starting_main_sha"] == "e39a7d2a6fe9b3c76f2ea68e1b32cdce9cfa8812"
