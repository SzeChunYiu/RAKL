...
def test_closure_contract_preserves_negative_completion_semantics() -> None:
    contract = json.loads(Path("research/self_rakl_closure_v1/CLOSURE_CONTRACT.json").read_text(encoding="utf-8"))
    ...
    arn_v1 = next(row for row in contract["wave_1_current"] if row["item"] == "arn-local-vs-parent-discriminator-v1")
    assert arn_v1["status"] == "REFUTED_AS_INSTRUMENT__DISTRACTOR_DESIGN_ARTIFACT"
