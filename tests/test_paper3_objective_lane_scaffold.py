"""Paper II objective-lane lifecycle and artifact contract (#444)."""
...
def test_objective_lane_artifacts_exist_after_confirmatory_completion() -> None:
    required = [
        "PROTOCOL.md",
    ...
    assert generator["status"] == "CONFIRMATORY_OBJECTIVE_COMPLETE__ROBUSTNESS_EXTERNAL_PENDING"
    assert generator["development"]["outcomes_accessed"] is True
    assert generator["confirmatory"]["items_generated"] == 576
    assert generator["confirmatory"]["outcomes_accessed"] is True
    assert generator["confirmatory"]["total_n"] == 576
    assert generator["grants_scientific_authority"] is False

    # The historical freeze receipt must remain unchanged evidence that the design
    # predated outcome access, even after the lane advances.
    ...
    # Legacy empty placeholders are retained as historical aliases; canonical
    # full synthetic packets are byte-reconstructed by the committed runner and
    # bound by CONFIRMATORY_PACKET_MANIFEST_V1.json.
    ...
    assert lane["objective"] == "CONFIRMATORY_OBJECTIVE_COMPLETE__ROBUSTNESS_EXTERNAL_PENDING"
    assert lane["objective_confirmatory_outcomes_accessed"] is True
