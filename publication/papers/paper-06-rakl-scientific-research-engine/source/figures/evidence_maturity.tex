% Standalone Orion evidence-maturity traffic light (Paper VI): honest status of evidence classes.
...
\node[item, text=oigreen!45!black] at ([yshift=-2mm]b1.north)
     {\textbf{mechanics + known-answer contracts}\\[3pt]
      $\bullet$ \emph{wired-in core:} authority projection, typed router, replay-verified operational map, epistemic-noninterference + integration-path mutations (executable)\\[2pt]
      ...
      $\bullet$ \texttt{SEARCH$\cdot$JUMP$\cdot$GLUE$\cdot$LIFT} router runs (proposal-only)\\[2pt]
      $\bullet$ \emph{additive sidecars (semantic-quotient assurance, structural-transfer use, unified-integration manifest):} contract + unit-tested, \textbf{wiring in progress} --- no production caller routes through them yet};
