% Standalone Orion provenance map (Paper VI): Papers I-V -> engine components.
...
\node[paper] (p3) at (0,-2*\ry)  {Paper III\\{\scriptsize Self-Orion}};
...
\node[comp] (c3) at (7.6,-2*\ry) {Self-Orion: experience $\to$ method\\+ four-arm method-change receipt};
...
\node[lbl, text=oiblue!70, anchor=south east] at ([yshift=1pt]engine.north east)
      {Orion research engine \ (Paper VI capstone) \ };
