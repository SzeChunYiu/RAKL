...
\caption{How this capstone integrates the series. Paper~I supplies the multi-axis authority poset $\alpha=(G,R,M,I,D)$ and the bounded workspace; Paper~II the directional transfer witness (\texttt{JUMP}) and typed transitions; Paper~III the Self-RAKL experience$\to$method loop and four-arm attribution; Paper~V the verified-discovery mathematical path (\texttt{SEARCH}$\cdot$\texttt{JUMP}$\cdot$\texttt{GLUE}$\cdot$\texttt{LIFT}). Paper~IV's training-time projection $\pi_{\mathrm{train}}$ is an \emph{optional}, decision-gated surface. The three projections share canonical identity and negative history but not authority semantics: training utility $\neq$ search rank $\neq$ scientific authority.}
