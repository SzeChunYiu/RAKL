...
We do not claim novelty for multi-agent science, knowledge graphs, provenance, sheaf-like local-to-global reasoning, hierarchical memory, prompt compression, active experiment design, causal discovery, skill libraries, program evolution, transactional belief commit, shared-memory governance or generic self-improvement in isolation. The candidate contribution is the integrated evidence-governed transition discipline connecting them, narrowed further by the closest-parent audit in Section~\ref{sec:intellectual-lineage}.

A novelty scan against the contemporary literature confirms that the surviving claim is the specific combination rather than any named primitive taken singly. Three of the conjunction's conjuncts are realized as standalone primitives by established or concurrent work: verifier-induced reachability over a symbolic deduction engine is the core mechanism of AlphaGeometry \cite{alphageometry2024} and of AlphaZero-style proof search \cite{htps2022}; the quotient of equivalence classes of solution alternatives $T(\Sigma)/{\approx}$ is exactly an equality-saturation e-class \cite{egg2021}; and concurrent agent-system work realizes the authority-noninterference and cross-stage-identity primitives cited at the corresponding mechanic descriptions in Sections~\ref{sec:unified_assurance_sidecars} and~\ref{sec:training-time-projection}. \RAKL{}'s residual is therefore not any one of these as a new primitive: it is the content-hash-bound, staleness-checked, typed and proposal-only \emph{configuration} of them behind a single authority firewall. The named constructs below (verified transformation geometry, the task-conditioned quotient, the structural-identity bridge) are RAKL-specific instantiations of these parents; the acronyms are RAKL-coined labels for the configuration, not claims of a new underlying mechanism.
...
\node[strong,text width=31mm,below=22mm of proposal] (meta) {Challenge Learning /\\Self-Orion};
...
\end{tikzpicture}}
\caption{\textbf{Orion separates proposal generation from scientific authority.} Evidence and tools inform a replaceable proposer, but candidate claims and method changes pass through explicit verification before canonical state can change. Residuals drive scientific fibers; method residuals can trigger Challenge Learning and Self-Orion, but fresh assurance is outside the challenger's write authority.}
\label{fig:architecture}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/four_loop_master.pdf}
\caption{The engine as four coupled loops behind one authority firewall. Information$\to$knowledge, problem$\to$solution, experience$\to$method and Orion$\to$better-Orion each run as proposal dynamics on the left (access, not authority); only an evidence- and governance-bearing certificate (a verification, fresh-assurance or protected Self-Orion gate) crosses the firewall to update canonical state on the right. Each loop's residual reopens a fibre; each loop's product couples into the next; the fourth loop's governed recursion may reopen loops~1--3. Persistence, retrieval, workspace occupancy, reflection and self-modification confer access but never scientific authority.}
\label{fig:fourloop}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/papers_integration_map.pdf}
\caption{How this capstone integrates the series. Paper~I supplies the multi-axis authority poset $\alpha=(G,R,M,I,D)$ and the bounded workspace; Paper~II the directional transfer witness (\texttt{JUMP}) and typed transitions; Paper~III the Self-Orion experience$\to$method loop and four-arm attribution; Paper~V the verified-discovery mathematical path (\texttt{SEARCH}$\cdot$\texttt{JUMP}$\cdot$\texttt{GLUE}$\cdot$\texttt{LIFT}). Paper~IV's training-time projection $\pi_{\mathrm{train}}$ is an \emph{optional}, decision-gated surface. The three projections share canonical identity and negative history but not authority semantics: training utility $\neq$ search rank $\neq$ scientific authority.}
...
Figure~\ref{fig:evidencematurity} states the evidence maturity of the engine at a glance, so that the empirical headline is read as \emph{pending} rather than claimed. The architecture and integration invariants are executed; efficacy, safety and cost claims are same-context-only or blocked behind a documented model-capability floor and separately registered experiments.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/evidence_maturity.pdf}
\caption{Evidence maturity of the engine. \textbf{Executed (wired-in core):} the immutable authority projection and certificate-bound promotion path, the typed router, operational-map replay-verified routing, the path-equivalence and epistemic-noninterference invariants, and the deterministic known-answer/hostile stress worlds (all proposal-only). \textbf{Contract + unit-tested, wiring in progress (additive sidecars):} the semantic-quotient assurance, structural-transfer use, and unified-integration manifest gates are defined with fail-closed contracts and pass their own unit tests, but no production caller currently routes through them; propositions relying on those gates hold only under their stated caller obligations until the gates are wired into the live call graph. \textbf{Same-context only:} interface self-repair (development-time), which is useful signal but not independent review or a superiority result. \textbf{Pending/blocked:} matched-workflow superiority, fresh-assurance self-evolution, a real quant-finance result, a fair external-LLM comparator and independent external review. The master blocker is a model-capability floor; no empirical-superiority claim is made in this paper.}
\label{fig:evidencematurity}
\end{figure}
