...
Although current LLM-based scientific agents make this failure mode especially visible, the mechanics studied here are not LLM-specific. \Framework{} is the executor-independent calculus of evidence-governed research transitions; \RAKL{} is its reference persistent substrate. A human researcher, deterministic algorithm, theorem prover, learned model or hybrid workflow may propose the next object. Executor identity changes provenance, cost and the available proposal distribution, but it does not change the conditions under which scientific authority may move.
...
\caption{Orion as a state machine around a proposal generator $G_\theta$. In current scientific-agent deployments the generator may be an LLM, but the state-transition and authority semantics do not depend on the executor class. The generator and open-world mechanism discovery emit \emph{proposals} into a capacity-bounded workspace; these gain computational access but carry no write authority. A typed verifier evaluates a proposal, and only the resulting verified certificate crosses the write-authority boundary to update the canonical state through an append-only authority ledger, with refuted claims retained as negative history rather than discarded. A saturation check governs when the process may stop and when unresolved threads are reopened.}
...
\item It states what the accumulated structure is \emph{for}: navigation of a minimal, obstruction-preserving support structure toward a registered target contract. A distillation that retains only pairwise compatibility data provably cannot decide global realizability, so obstruction objects must travel with the retained cores; failed navigation returns an epistemic cut naming what would have to be established rather than a bare failure.
\end{enumerate}

Seventeen of the paper's eighteen numbered formal claims are machine-checked in a dependency-free Lean development included in the public artifact; the kernel reports every theorem in that development as axiom-free. The remaining claim, the reservation-first greedy optimality theorem, has its key proof ingredients machine-checked but not their assembly, and machine-checking is not claimed for it. The mechanization posture is summarized in the reproducibility appendix (Appendix~\ref{app:reproducibility}). A machine-checked proof removes dependence on any human reviewer's line-by-line judgement; it is not independent peer review of the paper.
...
\item[Access, coherence, authority] the three levels the paper keeps separate: whether a claim is \emph{active/reusable} by computation (access), whether it can \emph{coexist} with others after their contexts are aligned (coherence), and what the registered evidence \emph{licenses the system to assert} (authority). These are the terms used hereafter; ``available/credible/true'' are informal glosses only.
\item[Proposal vs.\ canonical update] a model output is a \emph{proposal} with no write permission; the \emph{canonical} record changes only through a registered, verified transition.
\item[Authority vector $(G,R,M,I,D)$] the five certified coordinates of a claim's authority: grounding/provenance, representation/relation, mechanism, identification/bounding, decision-usability. It is a vector, not a single score.
\item[Workspace] a capacity-bounded, transient view that changes what downstream operators \emph{see} without granting any canonical write permission (access, not authority).
\item[Epistemic projection] the slice of the full system state that is allowed to carry scientific authority, as opposed to memory or workspace views.
\item[Compatibility complex vs.\ lattice] the claim graph is deliberately \emph{not} called a lattice unless a closure operator supplies the meet/join guarantees; the weaker default object is a typed compatibility complex.
\item[Gluing / global section] a \emph{global section} is a single assignment consistent with all local claims at once; \emph{gluing} is assembling local pieces into one. Pairwise-compatible pieces need not glue.
\item[Fiber] an open research thread or sub-question kept explicit until resolved.
\item[Chart / atlas] a claim scoped to a domain, representation and regime (chart); the collection of them (atlas). Used here as an obligation structure, not a full sheaf implementation.
\item[Bounded epistemic saturation] a principled stop condition: halt when repeated, deliberately varied search and review passes stop adding anything substantive under a fixed basis and freshness horizon --- not a claim of completeness.
\item[Non-compensatory growth vector] progress is a vector of distinct event types; gains on one coordinate cannot offset gaps on another, so there is no single ``knowledge score.''
\item[OWMD; SEARCH/JUMP/GLUE/LIFT] open-world mechanism discovery searches for a needed capability by its functional signature across diverse routes: reuse same-domain (SEARCH), transport cross-domain (JUMP), compose partial results (GLUE), or specify a missing operator (LIFT).
\end{description}

\section{Foundations and the gap between them}\label{sec:foundations}
...
That last requirement is especially important. A system that never stops is unusable; a system that stops because it has stopped asking new kinds of questions is unreliable. The saturation construction in Section~\ref{sec:saturation} treats stopping as an epistemic certificate rather than an intuition about having ``read enough.''

\section{Epistemic state and transition mechanics}

We now define the persistent state on which the later compatibility, workspace, discovery and saturation results operate. Figure~\ref{fig:loop} shows how these components are exercised as a loop of typed transitions before we make each one precise.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/recursive_loop.pdf}
\caption{The research process as a loop of typed transitions. Proposals from the generator and open-world discovery enter a bounded workspace and are verified; a verified certificate promotes or archives the claim into canonical state, while a refuted proposal is retained in negative history rather than discarded. Each round updates the non-compensatory growth vector $\Delta$. The process releases only when $\Delta$ is flat under a fixed measurement basis and freshness horizon (bounded saturation, Section~\ref{sec:saturation}); otherwise it reopens the implicated research fibers and continues. Each arrow corresponds to one of the registered transition classes.}
\label{fig:loop}
\end{figure}
...
\]
when certificate inclusion is the relevant local order. More general coordinate-specific entailment relations can replace set inclusion without changing the product-order argument in Section~\ref{sec:authority}.
...
\begin{proposition}[Proposal non-sovereignty]\label{prop:proposal-non-sovereignty}
