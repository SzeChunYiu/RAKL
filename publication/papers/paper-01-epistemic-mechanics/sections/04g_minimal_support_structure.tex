...
The navigation loop of this section is executable in the accompanying artifact, not only specified. A distilled structure (\path{support_solver.py}) carries atoms, authority-licensed typed edges and \emph{explicit obstruction objects}; route search rejects any assembled atom set realizing a registered obstructed cover --- the check is applied to the assembled cover rather than edge-locally, which is exactly the discipline Proposition~\ref{prop:obstruction-blind} forces --- and a failed search returns an epistemic cut (what no admissible route can avoid) together with a distinct minimal repair (what would suffice to open one), reporting when either set is approximate rather than exact. Multi-premise derivation is forward closure over hyperedges under the same obstruction refusal (\path{derivation.py}); as a real-data check, the dependency graph of the paper's own Lean development is parsed into hyperedges and a theorem is re-derived from the axiom-free base by traversal alone. Edge licenses are derived from certificates whose checkers are re-executed at use time: a failing checker demotes the edge and an erroring checker yields cannot-check, never a pass (\path{certificates.py}). A structure space with fail-closed applicability matching and recursive fiber decomposition composes these into a solving loop (\path{structure_space.py}, \path{recursive_solver.py}). Every output of this loop is a support claim carrying no scientific authority, per Section~\ref{sec:solver_noninterference}. Whether navigating distilled structures improves problem-solving performance over navigating the undistilled sources is under prospective evaluation at the time of writing; no benefit result is claimed here.
\end{remark}
