...
Scientific archives must also tolerate unresolved contradiction. Argumentation frameworks explicitly model attack and defense rather than treating the knowledge base as a single classical theory \cite{dung1995}; four-valued logics provide semantics in which inconsistent information can be represented without trivializing every proposition \cite{belnap1977}. Orion adopts the engineering lesson: conflicting claims can coexist as typed objects while their contradiction is recorded as an obstruction. The canonical state does not infer arbitrary consequences from the presence of a conflict.
