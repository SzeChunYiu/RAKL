...
\paragraph{Example: ascent from mechanic search to an information contract.}
A persistent acquisition negative can localize above the current mechanic family.  In
our structural-transfer programme, inspecting an untouched benchmark block exposed a
mix of source-grounded, semantic-normalization, and evaluator-contract cases.  The
recursive controller therefore ascends from repeated reducer search to the ancestor
question and freezes an observation contract before further execution.  This case
illustrates the existing non-sovereign \texttt{ASCEND}/\texttt{AUDIT\_EVALUATOR}
semantics; it does not introduce an additional recursion layer or authority path.
