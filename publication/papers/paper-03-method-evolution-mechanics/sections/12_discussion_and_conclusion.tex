...
\subsection{Prospective generative mechanics (proposal-only)}
\label{sec:prospective-generative-mechanics}

The mechanics in this paper evolve \emph{methods}: they diagnose a failed research episode and propose a better operator or lesson. A natural generalization---which we state here as a registered research direction, not a result---is to evolve the \emph{solving mechanics themselves}. In that framing a provisional solver state is a tuple $Z_t=(G,R,D,S,K,M,O,H,V,C,E)$ over goal, representation, decomposition, scale, active knowledge, memory, operator basis, search policy, verification policy, compute allocation and residual, and a meta-controller selects a bounded, auditable \emph{mechanic-deficiency witness}---the solving-time analogue of the directional structural witness of the companion structural paper---that attributes an observed residual to a specific deficient coordinate (representation, decomposition, scale, verifier, model floor, and so on) rather than to an introspective opinion. A second, sharper direction replaces part of combinatorial path search with a \emph{solvability field}: construct a representation $\Phi$ over solver states in which a cheap local signal predicts verified global progress, i.e.\ $\langle d(a),-\nabla\Phi(z)\rangle>0 \Rightarrow \mathbb{E}[\Delta R_{\mathrm{root}}\mid a]>0$ often enough to reduce cost. This is deliberately a falsifiable hypothesis, not an assumed truth: its registered failure outcomes include the field acting as a false attractor, its construction cost exceeding the search it saves, and its usefulness holding only under oracle-selected representations. These directions inherit the same discipline as the rest of the paper---proposal-only until matched-resource experiments on cheap known worlds show a strict reason to promote them---and are specified in a companion proposal-only design programme that grants no scientific, method or promotion authority.
