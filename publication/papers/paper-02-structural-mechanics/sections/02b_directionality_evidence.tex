...
The transfer decision is deliberately non-compensatory. Writing
$T=\operatorname{Transfer}(A\to B;q)$:
\begin{align*}
T&=\mathrm{LICENSED}
&&\text{if every load-bearing obligation is satisfied},\\
T&=\mathrm{REJECTED}
&&\text{if any load-bearing obligation is demonstrably violated},\\
T&=\mathrm{CANNOT\_CHECK}
&&\text{otherwise (any load-bearing obligation remains unresolved)}.
\end{align*}
