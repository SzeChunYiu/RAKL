...
\section{Directional witnesses as Orion experience-transfer relations}
\label{sec:v3-transfer}

Orion makes experience persistence an implemented architectural fact but leaves transfer validity as an evidence question. A source \texttt{TaskEpisode} says what happened in one registered context. A versioned \texttt{Lesson} is a scoped abstraction over one or more episodes. Neither object is automatically applicable to a new target. The role of the directional structural witness is to supply an explicit candidate relation
\[
W_{s\rightarrow t}^{(q)}:
(E_s,L_s,\gamma_s)
\rightsquigarrow
