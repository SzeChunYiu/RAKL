...
STAGING_USER_AGENT = "RAKL-Paper2-Staging-V3/2"
_CONTRACT_SELF_HASH_MODE = "canonical_contract_with_self_sha256_zeroed"
_CONTRACT_SELF_HASH_SENTINEL = "0" * 64


class ArtifactDownloadError(RuntimeError):
    """Typed download failure retaining only frozen artifact identity and status."""

    def __init__(self, artifact: Mapping[str, object], cause: Exception) -> None:
        self.artifact_id = str(artifact.get("artifact_id", ""))
        self.artifact_url = str(artifact.get("url", ""))
        status = getattr(cause, "code", None)
        self.http_status = int(status) if isinstance(status, int) else None
        super().__init__(f"artifact download failed:{self.artifact_id}: {cause}")
...
def _download_artifact(
    ...
    *,
    ...
    try:
        with opener(request, timeout=1800) as response, destination.open("wb") as output:
            shutil.copyfileobj(response, output, length=1024 * 1024)
    except Exception as exc:
        raise ArtifactDownloadError(artifact, exc) from exc
...
def _contract_self_sha256(contract: Mapping[str, object]) -> str:
    """Hash the complete contract with only its recursive digest zeroed.

    This creates a deterministic, locally checkable self-binding without the
    impossible requirement that a file contain its own ordinary byte hash.
    The binding path, role, hash mode, and every other contract field remain
    inside the hash preimage.
    """
    ...
    for binding in bindings:
        if isinstance(binding, dict) and binding.get("role") == "contract_self":
            binding["sha256"] = _CONTRACT_SELF_HASH_SENTINEL
            found += 1
    return _canonical_sha256(normalized) if found == 1 else ""
...
def validate_staging_contract(
    ...
    failures: list[str] = []
    if contract.get("schema_version") != "paper2-cpu-staging-contract-v3.2":
        failures.append("contract_schema_invalid")
    if contract.get("contract_id") != "PAPER2_CPU_STAGING_V3_2":
        failures.append("contract_id_invalid")
    if contract.get("supersedes_contract_id") != "PAPER2_CPU_STAGING_V3_1":
        failures.append("contract_supersession_invalid")
    predecessor = contract.get("predecessor_failure")
    if (
        not isinstance(predecessor, Mapping)
        or predecessor.get("contract_id") != "PAPER2_CPU_STAGING_V3_1"
        or predecessor.get("probe_job_id") != "3475098"
        or predecessor.get("staging_job_id") != "3475099"
        or predecessor.get("staging_verdict") != "STAGING_FAILED_PRESERVED"
        or predecessor.get("error_type") != "ValueError"
        or predecessor.get("error_detail") != "archive unsafe member:python/bin/2to3"
        or predecessor.get("preserved_candidate_path")
        != "/projects/hep/fs9/users/scyiu/RAKL-paper2/assets/.paper2-cpu-v3-1-candidate-3475099"
        or predecessor.get("archive_observation_verdict")
        != "ALL_38_ARTIFACTS_EXACT__ARCHIVE_RELATIVE_LINK_POLICY_MISMATCH_LOCALIZED"
    ):
        failures.append("predecessor_failure_lineage_invalid")
    ...
    if (
        ...
        or failure_policy.get("preserve_failed_candidate_and_receipt") is not True
        or failure_policy.get("preserve_v3_failure_lineage") is not True
        or failure_policy.get("preserve_v3_1_failure_lineage") is not True
    ):
        failures.append("failure_preservation_policy_missing")
    archive_policy = contract.get("archive_extraction_policy")
    required_archive_policy = {
        "prevalidate_complete_archive_before_write",
        "reject_absolute_member_paths",
        "reject_member_path_escapes",
        "reject_duplicate_normalized_names",
        "reject_link_ancestor_collisions",
        "reject_devices_fifos_and_unknown_types",
        "reject_absolute_link_targets",
        "reject_missing_link_targets",
        "reject_link_cycles",
        "reject_normalized_link_target_escapes",
        "permit_relative_symlinks_with_in_root_targets",
        "permit_parent_relative_symlinks_only_with_in_root_targets",
        "permit_hardlinks_only_through_acyclic_archive_member_chains_to_regular_files",
        "extract_regular_content_before_links",
        "materialize_links_without_following_unvalidated_filesystem_paths",
    }
    if not isinstance(archive_policy, Mapping) or any(
        archive_policy.get(field) is not True for field in required_archive_policy
    ):
        failures.append("archive_extraction_policy_invalid")
    ...
    if isinstance(candidate_root, str) and isinstance(final_root, str):
        ...
        if "v3-2" not in candidate_root or "v3-2" not in final_root:
            failures.append("v3_2_paths_not_distinct")
    ...
    for binding in bindings:
        ...
        elif role == "contract_self":
            if binding.get("sha256_mode") != _CONTRACT_SELF_HASH_MODE:
                failures.append("contract_self_hash_mode_invalid")
            elif not _hex(binding.get("sha256"), 64) or binding.get("sha256") != _contract_self_sha256(contract):
                failures.append("contract_binding_hash_mismatch:contract_self")
        ...
        elif binding.get("role") == "archive_observation":
            try:
                archive_observation = json.loads(path.read_text(encoding="utf-8"))
            except json.JSONDecodeError:
                failures.append("archive_observation_json_invalid")
    required_roles = {
        ...
        "submission_receipt_schema", "network_probe_receipt_schema",
        "result_receipt_schema", "harvest_receipt_schema", "archive_observation",
        "archive_observation_schema", "repo_bootstrap_receipt_schema", "contract_self",
    }
    ...
    expected_observation = {
        "schema_version": "paper2-v3-1-post-failure-archive-observation-v1",
        "subject_sha": "9d6ee25c9526cdf604bfeb727eeb6e1870cae16f",
        "probe_job_id": "3475098",
        "staging_job_id": "3475099",
        "staging_verdict": "STAGING_FAILED_PRESERVED",
        "archive_sha256": _EXPECTED_PYTHON["sha256"],
        "artifact_count": 38,
        "identity_matching_artifact_count": 38,
        "candidate_exists": True,
        "final_exists": False,
        "archive_member_count": 5400,
        "regular_file_count": 4352,
        "directory_count": 0,
        "symlink_count": 1048,
        "hardlink_count": 0,
        "parent_relative_link_count": 300,
        "special_member_count": 0,
        "all_link_targets_resolve_inside_archive_root": True,
        "all_link_targets_exist": True,
        "all_link_targets_are_regular_files": True,
        "candidate_mutation_performed": False,
        "jobs_submitted_by_observation": 0,
        "model_execution_performed": False,
        "evaluated_result_record_count": 0,
        "verdict": "ALL_38_ARTIFACTS_EXACT__ARCHIVE_RELATIVE_LINK_POLICY_MISMATCH_LOCALIZED",
        "v3_2_frozen_repair_requirement": (
            "Permit only relative symlink/hardlink members whose normalized resolved target "
            "remains inside the extraction root and resolves through an acyclic archive-member "
            "chain to an allowed file or directory. Continue rejecting absolute links, root "
            "escapes, devices, FIFOs, unknown types, missing targets and cycles."
        ),
    }
    if not isinstance(archive_observation, Mapping) or any(
        archive_observation.get(key) != value for key, value in expected_observation.items()
    ):
        failures.append("archive_observation_lineage_invalid")
    return tuple(dict.fromkeys(failures))
...
def build_submission_receipt(
    *,
    ...
    ) else repository_root / "research/paper2_microtrial_v3/CPU_STAGING_CONTRACT_V3_2.json"
...
def _normalized_archive_path(raw: str, *, member_name: str) -> PurePosixPath:
    """Normalize an archive path without consulting or mutating the filesystem."""

    if not raw or "\x00" in raw or PurePosixPath(raw).is_absolute():
        raise ValueError(f"archive unsafe member:{member_name}")
...
def _safe_extract(archive: Path, destination: Path) -> None:
    """Extract a tarball only after a complete archive-graph validation pass.

    The validation phase is filesystem-write free.  It treats links as edges
    between normalized archive members, rejects every unsafe/cyclic graph, and
    records the terminal regular member for hardlinks.  The write phase creates
    directories and regular files before any links, so no archive-controlled
    link can be followed while regular content is written.
    """

    try:
        bundle = tarfile.open(archive, "r:gz")
    except (OSError, tarfile.TarError) as exc:
        raise ValueError("archive unsafe or unreadable archive") from exc
    with bundle:
        members = bundle.getmembers()
        entries: dict[PurePosixPath, tuple[tarfile.TarInfo, str]] = {}
        link_targets: dict[PurePosixPath, PurePosixPath] = {}

        # Phase 1A: normalize every member and classify only the allowed types.
        for member in members:
            name = _normalized_archive_path(member.name, member_name=member.name)
            if name in entries:
                raise ValueError(f"archive unsafe duplicate member:{member.name}")
            if member.type in {tarfile.REGTYPE, tarfile.AREGTYPE}:
                ...
                # Includes devices, FIFOs, sparse/contiguous and unknown types.
                raise ValueError(f"archive unsafe member type:{member.name}")
            entries[name] = (member, kind)

        # Phase 1B: reject any non-directory archive member used as an ancestor.
        # This catches link-ancestor collisions independent of member ordering.
        for name in entries:
            for index in range(1, len(name.parts)):
                ancestor = PurePosixPath(*name.parts[:index])
                ancestor_entry = entries.get(ancestor)
                if ancestor_entry is not None and ancestor_entry[1] != "dir":
                    raise ValueError(f"archive unsafe ancestor:{ancestor}")

        # Phase 1C: normalize every relative link target and require an exact
        # archive-member target.  Symlinks are relative to their own parent;
        ...
        for name, (member, kind) in entries.items():
            ...
            if not member.linkname or PurePosixPath(member.linkname).is_absolute():
                raise ValueError(f"archive unsafe link:{member.name}")
            raw_target = (
                posixpath.join(str(name.parent), member.linkname)
                if kind == "symlink"
                else member.linkname
            )
            target = _normalized_archive_path(raw_target, member_name=member.name)
            if target not in entries:
                raise ValueError(f"archive unsafe missing link target:{member.name}")
            link_targets[name] = target

        # Phase 1D: resolve the complete archive-member link graph.  Every link
        # must terminate at an allowed file/directory and every cycle fails.
        terminal: dict[PurePosixPath, tuple[PurePosixPath, str]] = {}

        def resolve(name: PurePosixPath, active: frozenset[PurePosixPath]) -> tuple[PurePosixPath, str]:
            cached = terminal.get(name)
            if cached is not None:
                return cached
            if name in active:
                raise ValueError(f"archive unsafe link cycle:{name}")
            _member, kind = entries[name]
            if kind in {"file", "dir"}:
                result = (name, kind)
            else:
                result = resolve(link_targets[name], active | {name})
            terminal[name] = result
            return result

        for name, (_member, kind) in entries.items():
            if kind in {"symlink", "hardlink"}:
                terminal_name, terminal_kind = resolve(name, frozenset())
                # POSIX filesystems cannot materialize a hardlink to a directory.
                if kind == "hardlink" and terminal_kind != "file":
                    raise ValueError(f"archive unsafe hardlink target:{name}")
                terminal[name] = (terminal_name, terminal_kind)

        # The destination itself is part of the safety boundary.  Existing
        # content or a destination symlink could reintroduce an unvalidated path.
        if destination.is_symlink() or (destination.exists() and not destination.is_dir()):
            raise ValueError("archive unsafe extraction destination")
        if destination.exists() and any(destination.iterdir()):
            raise ValueError("archive unsafe nonempty extraction destination")

        # Phase 2: writes begin only after the whole archive graph passes.
        destination.mkdir(parents=True, exist_ok=True)
        directories = sorted(
            (name for name, (_member, kind) in entries.items() if kind == "dir"),
        ...
        for name, (member, kind) in entries.items():
            ...
            if source is None:
                raise ValueError(f"archive unsafe unreadable member:{member.name}")
            with source, target.open("xb") as output:
                shutil.copyfileobj(source, output, length=1024 * 1024)
            target.chmod(member.mode & 0o755)

        # Links are materialized last.  Symlink creation never dereferences the
        # target.  Hardlinks use the already validated terminal regular file,
        # not an intermediate archive-controlled link path.
        for name, (member, kind) in entries.items():
            if kind not in {"symlink", "hardlink"}:
                continue
            target = destination / Path(*name.parts)
            ...
            if kind == "symlink":
                ...
                terminal_name, _terminal_kind = terminal[name]
                source = destination / Path(*terminal_name.parts)
...
def _stage_assets_impl(*, contract: Mapping[str, object], repository_root: Path, expected_repo_sha: str, probe_receipt_path: Path) -> dict[str, Any]:
    ...
    if failures:
        receipt = {"schema_version":"paper2-cpu-staging-result-receipt-v3.2","created_at_utc":_now(),"contract_canonical_sha256":_canonical_sha256(contract),"expected_repo_sha":expected_repo_sha,"repository_attestation":repo_attestation,"probe_receipt_path":str(probe_receipt_path),"probe_receipt_sha256":_sha256(probe_receipt_path) if probe_receipt_path.is_file() else None,"probe_slurm_job_id":probe_job_id,"slurm_job_id":os.environ.get("SLURM_JOB_ID"),"verdict":"STAGING_REFUSED","failures":list(dict.fromkeys(failures)),"candidate_path":str(candidate),"candidate_preserved":candidate.exists(),"final_path":str(final),"final_exists":final.exists(),"model_execution_performed":False,"evaluated_result_record_count":0}
    ...
    try:
        ...
        receipt = {"schema_version":"paper2-cpu-staging-result-receipt-v3.2","created_at_utc":_now(),"contract_canonical_sha256":_canonical_sha256(contract),"expected_repo_sha":expected_repo_sha,"repository_attestation":repo_attestation,"probe_receipt_path":str(probe_receipt_path),"probe_receipt_sha256":_sha256(probe_receipt_path),"probe_slurm_job_id":probe_job_id,"slurm_job_id":os.environ.get("SLURM_JOB_ID"),"verdict":"STAGING_PASS_ATOMICALLY_PROMOTED","authority_condition":"This pass verdict is authoritative only when this receipt is harvested from final_path after the atomic candidate rename succeeds.","failures":[],"candidate_path":str(candidate),"final_path":str(final),"artifact_count":len(observed_files),"observed_files":observed_files,"installed_versions":installed,"installed_distributions":installed_distributions,"pip_check_returncode":pip_check.returncode,"pip_check_stdout":pip_check.stdout.strip(),"pip_freeze_all":pip_freeze_all,"torch_cpu_smoke":torch_smoke,"standalone_python_smoke":python_smoke,"platform_receipt":platform_observation,"fs9_disk_usage_before_staging":{"total":disk.total,"used":disk.used,"free":disk.free},"model_execution_performed":False,"evaluated_result_record_count":0}
        final.parent.mkdir(parents=True, exist_ok=True)
        _commit_staging_candidate(candidate, final, receipt)
        return receipt
    except Exception as exc:
        download_error = exc if isinstance(exc, ArtifactDownloadError) else None
        candidate_preserved = candidate.exists()
        final_exists = final.exists()
        failure_verdict = (
            "STAGING_FAILED_PRESERVED"
            if candidate_preserved and not final_exists
            else "STAGING_FAILED_PRESERVATION_UNVERIFIED"
        )
        receipt = {"schema_version":"paper2-cpu-staging-result-receipt-v3.2","created_at_utc":_now(),"contract_canonical_sha256":_canonical_sha256(contract),"expected_repo_sha":expected_repo_sha,"repository_attestation":repo_attestation,"probe_receipt_path":str(probe_receipt_path),"probe_receipt_sha256":_sha256(probe_receipt_path) if probe_receipt_path.is_file() else None,"probe_slurm_job_id":probe_job_id,"slurm_job_id":os.environ.get("SLURM_JOB_ID"),"verdict":failure_verdict,"failures":["staging_exception"],"error_type":type(exc).__name__,"error_detail":str(exc),"failed_artifact_id":download_error.artifact_id if download_error else None,"failed_artifact_url":download_error.artifact_url if download_error else None,"http_status":download_error.http_status if download_error else None,"candidate_path":str(candidate),"candidate_preserved":candidate_preserved,"final_path":str(final),"final_exists":final_exists,"model_execution_performed":False,"evaluated_result_record_count":0}
...
def stage_assets(*, contract: Mapping[str, object], repository_root: Path, expected_repo_sha: str, probe_receipt_path: Path) -> dict[str, Any]:
    ...
    failure_root = Path(str(contract.get("failure_root", FS9_ROOT / "failures/v3_2")))
    try:
...
def build_harvest_receipt(
    *,
    ...
    failure_root: Path,
    runner: Callable[..., Any] = subprocess.run,
    git_observer: Callable[[Path], tuple[str, bool]] = _git_observation,
    path_mapper: Callable[[str], Path] = Path,
) -> dict[str, Any]:
    failures: list[str] = []
    contract_failures = validate_staging_contract(contract, repository_root=repository_root)
    ...
    if any(
        path_mapper(str(contract.get(field, ""))) != supplied
        for field, supplied in governed_roots.items()
    ):
        failures.append("governed_root_mismatch")
    manifest_path = _binding_path(contract, "asset_manifest", repository_root)
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_artifacts = {
    ...
    try:
        submitted_source = json.loads(submission_receipt_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        pass
    if submitted_source != dict(submission_receipt):
        failures.append("submission_receipt_source_mismatch")
    ...
    if (
        not isinstance(bootstrap, Mapping)
        or bootstrap.get("schema_version") != "paper2-repo-bootstrap-v3"
        or bootstrap.get("verdict") not in {
            "BOOTSTRAP_PASS_EXISTING_EXACT_CHECKOUT",
            "BOOTSTRAP_PASS_ATOMICALLY_PROMOTED",
        }
        or bootstrap.get("exit_status") != 0
        or bootstrap.get("expected_repo_sha") != expected_sha
        or bootstrap.get("observed_repo_sha") != expected_sha
        or not _hex(bootstrap.get("observed_repo_tree"), 40)
        or bootstrap.get("repo_path") != str(repository_root)
        or bootstrap.get("github_remote") != "https://github.com/SzeChunYiu/RAKL.git"
        or bootstrap.get("checkout_clean") is not True
        or bootstrap.get("detached_head") is not True
        or bootstrap.get("jobs_submitted") != 0
        or bootstrap.get("model_execution_performed") is not False
        or bootstrap.get("evaluated_result_record_count") != 0
    ):
        failures.append("bootstrap_receipt_semantics_invalid")
    try:
        harvest_repo_sha, harvest_repo_clean = git_observer(repository_root)
    except Exception:
        harvest_repo_sha, harvest_repo_clean = "", False
    if harvest_repo_sha != expected_sha or not harvest_repo_clean:
        failures.append("harvest_repository_observation_invalid")
    ...
    expected_job_ids = [str(job) for job in jobs]
    row_counts = {job: sum(row.get("job_id") == job for row in rows) for job in expected_job_ids}
    scheduler_rows_exact = bool(
        len(expected_job_ids) == 2
        and len(set(expected_job_ids)) == 2
        and len(rows) == 2
        and all(row_counts.get(job) == 1 for job in expected_job_ids)
    )
    if not scheduler_rows_exact:
        failures.append("scheduler_root_rows_not_exactly_one_per_job")
    ...
    if (
        submission_receipt.get("schema_version")
        != "paper2-cpu-staging-submission-receipt-v3.2"
        or contract_sha != expected_contract_sha
        or not _hex(expected_sha, 40)
        or submission_receipt.get("observed_repo_sha") != expected_sha
        or submission_receipt.get("failures") != []
        or submission_receipt.get("model_execution_performed") is not False
        or submission_receipt.get("evaluated_result_record_count") != 0
    ):
        failures.append("submission_receipt_semantics_invalid")
    probe_observations = probe.get("observations") if probe else None
    probe_ids = (
        [str(item.get("artifact_id")) for item in probe_observations if isinstance(item, Mapping)]
        if isinstance(probe_observations, list)
        else []
    )
    probe_semantics_valid = bool(
        isinstance(probe_observations, list)
        and len(probe_observations) == len(manifest_artifacts) == 38
        and len(probe_ids) == len(set(probe_ids))
        and set(probe_ids) == set(manifest_artifacts)
        and all(
            isinstance(item, Mapping)
            and item.get("reachable") is True
            and isinstance(item.get("http_status"), int)
    ...
    if (
        probe is None
        or probe.get("schema_version") != "paper2-cpu-staging-network-probe-receipt-v3.2"
        or probe.get("verdict") != "NETWORK_PROBE_PASS"
        or probe.get("contract_canonical_sha256") != contract_sha
        or probe.get("expected_repo_sha") != expected_sha
        or probe.get("observed_repo_sha") != expected_sha
        or not jobs
        or str(probe.get("slurm_job_id")) != str(jobs[0])
        or probe.get("failures") != []
        or not probe_semantics_valid
        or probe.get("model_execution_performed") is not False
        or probe.get("evaluated_result_record_count") != 0
    ):
        failures.append("network_probe_receipt_invalid")
    scheduler_success = bool(
        scheduler_rows_exact
        and all(
    ...
    scheduler_negative = bool(
        scheduler_rows_exact
        and by_job.get(str(jobs[0]), {}).get("state") == "COMPLETED"
        and by_job.get(str(jobs[0]), {}).get("exit_code") == "0:0"
        and by_job.get(str(jobs[1]), {}).get("state", "").split("+", 1)[0] in terminal_states
        and not (
            by_job.get(str(jobs[1]), {}).get("state") == "COMPLETED"
            and by_job.get(str(jobs[1]), {}).get("exit_code") == "0:0"
        )
    ) if jobs else False
    observed_files = stage.get("observed_files") if stage else None
    observed_artifacts = (
        {str(item.get("artifact_id")): item for item in observed_files if isinstance(item, Mapping)}
        if isinstance(observed_files, list)
        else {}
    )
    observed_files_valid = bool(
        isinstance(observed_files, list)
        and len(observed_files) == len(observed_artifacts) == len(manifest_artifacts) == 38
        and set(observed_artifacts) == set(manifest_artifacts)
        and all(
            observed_artifacts[artifact_id].get("path") == expected["path"]
            and observed_artifacts[artifact_id].get("bytes") == expected["bytes"]
            and observed_artifacts[artifact_id].get("sha256") == expected["sha256"]
            for artifact_id, expected in manifest_artifacts.items()
        )
    )
    wheel_lock = json.loads(
        _binding_path(contract, "wheel_lock", repository_root).read_text(encoding="utf-8")
    )
    expected_distributions = {
        str(wheel["name"]).lower().replace("_", "-"): str(wheel["version"])
        for wheel in wheel_lock["wheels"]
    ...
    expected_distributions.update({"pip": "24.3.1", "setuptools": "75.6.0"})
    pip_freeze_all = stage.get("pip_freeze_all") if stage else None
    frozen_distributions: dict[str, str] = {}
    pip_freeze_valid = isinstance(pip_freeze_all, list)
    if pip_freeze_valid:
        for line in pip_freeze_all:
            if not isinstance(line, str) or line.count("==") != 1:
                pip_freeze_valid = False
                break
            name, version = line.split("==", 1)
            normalized_name = name.lower().replace("_", "-")
            if not name or not version or normalized_name in frozen_distributions:
                pip_freeze_valid = False
                break
            frozen_distributions[normalized_name] = version
    pip_freeze_valid = bool(pip_freeze_valid and frozen_distributions == expected_distributions)
    disk_observation = stage.get("fs9_disk_usage_before_staging") if stage else None
    minimum_free_bytes = int(
        contract.get("staging_attestation_policy", {}).get("minimum_free_bytes", 0)
    )
    disk_observation_valid = bool(
        isinstance(disk_observation, Mapping)
        and all(
            isinstance(disk_observation.get(field), int)
            and not isinstance(disk_observation.get(field), bool)
            and int(disk_observation[field]) >= 0
            for field in ("total", "used", "free")
        )
        and int(disk_observation["total"])
        == int(disk_observation["used"]) + int(disk_observation["free"])
        and int(disk_observation["free"]) >= minimum_free_bytes
    )
    expected_candidate_path = (
        final_root.parent / f".{final_root.name}-candidate-{jobs[1]}" if jobs else None
    )
    repository_attestation = stage.get("repository_attestation") if stage else None
    stage_success = bool(
        stage
        and stage.get("schema_version") == "paper2-cpu-staging-result-receipt-v3.2"
        ...
        and jobs
        and str(stage.get("slurm_job_id")) == str(jobs[1])
        and str(stage.get("probe_slurm_job_id")) == str(jobs[0])
        and stage.get("probe_receipt_path") == str(probe_path)
        and stage.get("probe_receipt_sha256") == (_sha256(probe_path) if probe_path.is_file() else None)
        and stage.get("candidate_path") == str(expected_candidate_path)
        and stage.get("final_path") == str(final_root)
        and final_root.is_dir()
        and stage.get("failures") == []
        and stage.get("artifact_count") == 38
        and observed_files_valid
        and stage.get("installed_versions") == _EXPECTED_RUNTIME
        and stage.get("installed_distributions") == expected_distributions
        and stage.get("pip_check_returncode") == 0
        and stage.get("pip_check_stdout") == "No broken requirements found."
        and pip_freeze_valid
        and disk_observation_valid
        and stage.get("torch_cpu_smoke") == {"version": "2.8.0+cpu", "cuda": None, "device": "cpu"}
        and isinstance(stage.get("standalone_python_smoke"), Mapping)
        and stage["standalone_python_smoke"].get("version") == "3.11.13"
        and stage["standalone_python_smoke"].get("executable")
        == str(expected_candidate_path / "runtime/python/bin/python3.11")
        and isinstance(stage.get("platform_receipt"), Mapping)
        and stage["platform_receipt"].get("architecture") == "x86_64"
        and isinstance(repository_attestation, Mapping)
        and repository_attestation.get("repo_sha") == expected_sha
        and repository_attestation.get("checkout_clean") is True
        and repository_attestation.get("construction_parent_sha")
        == contract.get("construction_parent_sha")
        and repository_attestation.get("construction_parent_ancestor") is True
        and _hex(repository_attestation.get("repo_tree_sha"), 40)
        and not existing_failure_paths
        and stage.get("model_execution_performed") is False
        and stage.get("evaluated_result_record_count") == 0
    )
    negative_common = bool(
        failed_stage
        and failed_stage.get("schema_version") == "paper2-cpu-staging-result-receipt-v3.2"
        ...
        and jobs
        and str(failed_stage.get("slurm_job_id")) == str(jobs[1])
        and str(failed_stage.get("probe_slurm_job_id")) == str(jobs[0])
        and failed_stage.get("probe_receipt_path") == str(probe_path)
        and failed_stage.get("probe_receipt_sha256") == (_sha256(probe_path) if probe_path.is_file() else None)
        and failed_stage.get("final_path") == str(final_root)
        and failed_stage.get("model_execution_performed") is False
        and failed_stage.get("evaluated_result_record_count") == 0
    )
    failed_preserved_valid = bool(
        negative_common
        and failed_stage.get("verdict") == "STAGING_FAILED_PRESERVED"
        and failed_stage.get("failures") == ["staging_exception"]
        and isinstance(failed_stage.get("error_type"), str)
        and bool(failed_stage.get("error_type"))
        and isinstance(failed_stage.get("error_detail"), str)
        and bool(failed_stage.get("error_detail"))
        and failed_stage.get("candidate_preserved") is True
        and failed_stage.get("final_exists") is False
        and failed_stage.get("candidate_path") == str(expected_candidate_path)
        and expected_candidate_path is not None
        and expected_candidate_path.is_dir()
        and not final_root.exists()
    )
    refused_valid = bool(
        negative_common
        and failed_stage.get("verdict") == "STAGING_REFUSED"
        and isinstance(failed_stage.get("failures"), list)
        and bool(failed_stage.get("failures"))
        and failed_stage.get("candidate_path") == str(expected_candidate_path)
        and isinstance(failed_stage.get("candidate_preserved"), bool)
        and failed_stage.get("candidate_preserved")
        is Path(str(failed_stage.get("candidate_path", ""))).exists()
        and isinstance(failed_stage.get("final_exists"), bool)
        and failed_stage.get("final_exists") is final_root.exists()
    )
    stage_negative = failed_preserved_valid or refused_valid
    ...
    elif scheduler_negative and stage_negative and failures == ["staging_job_or_receipt_failed"]:
    ...
    return {
        ...
        "failures": list(dict.fromkeys(failures)),
        "negative_history_preserved": verdict == "HARVEST_STAGING_NEGATIVE_PRESERVED",
        "model_execution_performed": False,
        "evaluated_result_record_count": 0,
        "verdict": verdict,
    }
...
def main(argv: list[str] | None = None) -> int:
    parser=argparse.ArgumentParser(description="Paper 2 CPU staging V3.2; never executes the model")
