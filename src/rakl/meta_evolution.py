...
class EvolutionLayer(str, Enum):
    """Mutable layers of the RAKL/ORION problem-solving stack."""
...
class MethodGenome:
    """Versioned identifiers for method surfaces that may be challenged.

    Identifiers are open strings by design: a recursive/fractal topology, a
    semantic representation, an ontology, or a mutation DSL is an incumbent
    research object rather than a protected architectural truth.
    """
...
class GenomeMutation:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.rationale:
            raise ValueError("mutation rationale cannot be empty")
        if self.target_layer is EvolutionLayer.CONSTITUTION:
            raise ValueError(
                "constitutional amendments are not materialized as ordinary genome mutations"
            )
        if len({name for name, _ in self.predicted_qoi_deltas}) != len(
            self.predicted_qoi_deltas
        ):
...
def materialize_challenger_genome(
    ...
    *,
    ...
    return replace(
        ...
        **{field_name: mutation.replacement_id},
    )
...
class EvolutionPortrait:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(self.registered_topology_challengers)) != len(
            self.registered_topology_challengers
        ):
...
_CAUSE_LAYER: Mapping[MechanicCause, tuple[EvolutionLayer, ...]] = {
    ...
    MechanicCause.NO_LOCAL_GEOMETRY_IN_SCOPE: (
        EvolutionLayer.REPRESENTATION,
        EvolutionLayer.TOPOLOGY,
    ),
}
...
def plan_self_evolution(portrait: EvolutionPortrait) -> SelfEvolutionPlan:
    """Route weakness to the smallest justified mutable layer set.

    Repeated failure broadens the *kind* of search rather than only increasing
    retries in the incumbent basis.  The escalation is explicit and falsifiable:
    operator -> representation -> topology -> meta-policy -> mutation language.
    """
    ...
    for cause in portrait.causes:
        if cause is not MechanicCause.UNKNOWN:
...
class CandidateDelta:
    ...
    def vector(self) -> tuple[float, ...]:
        # Every coordinate is an improvement delta: larger is better.
...
def pareto_frontier(candidates: Iterable[CandidateDelta]) -> tuple[CandidateDelta, ...]:
    """Retain non-dominated challengers without a universal scalar utility."""
...
class MutationPolicy:
    ...
    def from_mapping(cls, weights: Mapping[str, float]) -> "MutationPolicy":
        return cls(
            tuple(sorted((str(name), float(weight)) for name, weight in weights.items()))
        )
...
def update_mutation_policy(
    ...
    *,
    ...
    """Assign evidence-derived credit to the method that generated a method change."""

    weights = policy.as_dict()
    if operator_id not in weights:
        raise ValueError(
            "operator must already be registered before policy credit assignment"
        )
    delta = {
...
class MutationGovernance:
    ...
    eligible_to_enter_promotion_gate: bool
    requires_outer_assurance: bool
    reasons: tuple[str, ...]

    @property
    def eligible_for_auto_promotion(self) -> bool:
        """Compatibility alias: this never means the gate itself is bypassed."""

        return self.eligible_to_enter_promotion_gate
...
def assess_mutation_governance(
    *,
    ...
    """Make broad mutability compatible with non-self-authorization."""

    if target_layer is EvolutionLayer.CONSTITUTION:
        return MutationGovernance(
            proposal_allowed=True,
            eligible_to_enter_promotion_gate=False,
    ...
    return MutationGovernance(
        proposal_allowed=True,
        eligible_to_enter_promotion_gate=True,
