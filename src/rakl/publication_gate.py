...
class PublicationGateVerdict(str, Enum):
    """Scoped engineering verdict for a paper submission package.

    This layer cannot establish scientific truth.  It only checks whether the
    registered evidence/review/artifact prerequisites for submission are
...
def evaluate_publication_gate(packet: PublicationGatePacket) -> PublicationGateReport:
    """Fail-closed submission-readiness classifier.

    High scientific/model performance can never compensate for a blocking
    validity failure or an unreceipted/manual headline result.
    """
    ...
    return PublicationGateReport(
        ...
        reasons=(
            "all registered result, fresh-assurance, independent-review and artifact gates are represented as satisfied",
        ),
