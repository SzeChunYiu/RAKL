...
STAGING_USER_AGENT = "RAKL-Paper2-Staging-V3/1"
...
def validate_staging_contract(
    ...
    failures: list[str] = []
    if contract.get("schema_version") != "paper2-cpu-staging-contract-v3.1":
        failures.append("contract_schema_invalid")
    if contract.get("contract_id") != "PAPER2_CPU_STAGING_V3_1":
        failures.append("contract_id_invalid")
    if contract.get("supersedes_contract_id") != "PAPER2_CPU_STAGING_V3":
        failures.append("contract_supersession_invalid")
    predecessor = contract.get("predecessor_failure")
    if (
        not isinstance(predecessor, Mapping)
        or predecessor.get("contract_id") != "PAPER2_CPU_STAGING_V3"
        or predecessor.get("probe_job_id") != "3475080"
        or predecessor.get("staging_job_id") != "3475081"
        or predecessor.get("staging_verdict") != "STAGING_FAILED_PRESERVED"
        or predecessor.get("http_status") != 403
        or predecessor.get("localization_authority")
        != "bounded_inference_from_first_missing_manifest_artifact_not_direct_failure-receipt_proof"
    ):
    ...
    if isinstance(candidate_root, str) and isinstance(final_root, str):
        ...
        if "v3-1" not in candidate_root or "v3-1" not in final_root:
            failures.append("v3_1_paths_not_distinct")
    ...
    required_roles = {
        ...
        "result_receipt_schema", "harvest_receipt_schema", "contract_self",
    }
...
def build_submission_receipt(
    *,
    ...
    receipt: dict[str, Any] = {
        "schema_version": "paper2-cpu-staging-submission-receipt-v3.1",
        "created_at_utc": _now(),
    ...
    ) else repository_root / "research/paper2_microtrial_v3/CPU_STAGING_CONTRACT_V3_1.json"
...
def run_network_probe(*, contract: Mapping[str, object], repository_root: Path, expected_repo_sha: str, receipt_output: Path) -> dict[str, Any]:
    ...
    receipt = {
        "schema_version": "paper2-cpu-staging-network-probe-receipt-v3.1",
...
def stage_assets(*, contract: Mapping[str, object], repository_root: Path, expected_repo_sha: str, probe_receipt_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        receipt = {
            "schema_version": "paper2-cpu-staging-result-receipt-v3.1",
            "created_at_utc": _now(),
...
def build_harvest_receipt(
    *,
    ...
    return {
        "schema_version": "paper2-cpu-staging-harvest-receipt-v3.1",
        "created_at_utc": _now(),
...
def main(argv: list[str] | None = None) -> int:
    parser=argparse.ArgumentParser(description="Paper 2 CPU staging V3.1; never executes the model")
