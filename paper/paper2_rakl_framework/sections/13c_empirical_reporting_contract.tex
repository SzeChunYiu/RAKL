...
The next frozen discriminator is deliberately narrower than the confirmatory
factorial.  One sealed known-answer pendulum task at seed 17 will compare two
prompt materializations, \texttt{DIRECT\_CORPUS} and
\texttt{RAKL\_CONTEXT}, using identical complete-sealed evidence, model,
questions, evaluator, no-tool policy and resource ceiling.  A successful native
job must emit two arm records plus one task--seed index binding the raw output,
...
model and one deterministic seed.  Its only purpose is to test the real inference
and receipt chain before expanding the task panel.  At the present source cutoff
the V4 packet is frozen but not merged into the governed LUNARC checkout, no V4
job has been submitted and no result or quantitative figure exists.  Exact status
and fail-closed counts are recorded in
\path{research/paper2_microtrial_v4/PAPER2_V4_FROZEN_BATCH_READINESS_RECEIPT_20260811.json}.
