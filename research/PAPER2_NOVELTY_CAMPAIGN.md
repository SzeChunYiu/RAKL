...
## Live scoreboard (2026-08-11, post-#238/#283/#285; see `research/PARALLEL_SCHEDULER_SNAPSHOT.md`)
...
| **#155** | 2×2 preregistration + G1/G2 schema (#273) | No cells executed | **No** | Blocked on #247 discriminative learning + capability; #154 scored ALR; execution coords still `PENDING_FREEZE_*` |
| **#156** | Function matrix + ladder (#275); A3↔A4 conformance (#300); **matched empirical packet+runner frozen** | Packet `EMPIRICS_UNRUN`; no model arm scores | **No** | AutoSci/`CANNOT_CHECK` disposition; model/policy responder payloads for benefit/cost + uncertainty |
...
| **#247** (#238 CLOSED) | Learning-loop repair + capability-floor correction landed | Powered staircase / selective RAKL still open | **No** | Discriminative verified-lesson path remains critical for #155/#157 |

Authoritative cross-paper graph remains #258. Scheduler snapshot: `research/PARALLEL_SCHEDULER_SNAPSHOT.md` / #284. Proposal-only issue closure is not evidence completion.
