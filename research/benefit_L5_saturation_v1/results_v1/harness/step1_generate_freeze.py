"""Step 1 — generate the known-answer authority-upgrade corpus and freeze it into
the RSHEA receipt chain BEFORE any arm executes
(PROTOCOL.json corpus.label_independence_safeguards[0]).
...
- audit_sample.json         40 seed-sampled worlds: surface text + gold ONLY
...
from common import (  # noqa: E402
    ...
    sha256_file, utc_now_iso, verify_frozen_evaluator, verify_module_pins,
    write_json,
)
...
ARM_INPUT_FIELDS = ("world_id", "rounds", "surface_text")
...
def main() -> int:
    ...
    errors = class_invariant_checks(corpus["worlds"])
    if errors:
        for e in errors[:20]:
    ...
    stripped = {
        ...
        "worlds": [{k: row[k] for k in ARM_INPUT_FIELDS} for row in corpus["worlds"]],
    }
    stripped_sha = write_json(stripped_path, stripped)

    rng_audit = random.Random(REGISTERED_SEED)
    by_id = {row["world_id"]: row for row in corpus["worlds"]}
    audit_ids = rng_audit.sample(sorted(by_id), 40)
    audit_sample = {
        "note": ("auditor-facing sample: ONLY the rendered claim description and the "
                 "gold label; plus gold basis/t_complete per protocol; no class field, no arm outputs exist yet"),
        "sample_seed": REGISTERED_SEED,
        "worlds": [
            {
                "world_id": cid,
                "surface_text": by_id[cid]["surface_text"],
                "gold_basis": by_id[cid]["gold_basis"],
                "t_complete": by_id[cid]["t_complete"],
                "rounds_rendered_stream": by_id[cid]["rounds"],
            }
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l5-saturation-v1:corpus-freeze",
        ...
        residual_after=("L5-benefit-unmeasured",),
        retained_novelty=(),
