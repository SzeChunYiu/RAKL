...
Also computes the per-class completeness/IoU/savings breakdown
...
_spec = importlib.util.spec_from_file_location("l5_frozen_evaluator_s3", EVALUATOR_PATH)
...
def per_class_breakdown(corpus_rows, arm_out_path):
    with open(arm_out_path, "r", encoding="utf-8") as handle:
        rows = json.load(handle)["stops"]
    stop = {d["world_id"]: d["stop_round"] for d in rows}
    result = {}
    for klass in CLASSES + ("all",):
        krows = (corpus_rows if klass == "all"
                 else [r for r in corpus_rows if r["class"] == klass])
        n = len(krows)
        if n == 0:
            result[klass] = None
            continue
        comp = sum(1 for r in krows if _frozen.complete(r, stop[r["world_id"]]))
        ious = [_frozen.iou(r, stop[r["world_id"]]) for r in krows]
        savings = [(T_MAX - stop[r["world_id"]]) / T_MAX for r in krows]
        result[klass] = {
            "n": n,
            "completeness": comp / n,
            "mean_iou": sum(ious) / n,
            "mean_stop_round": sum(stop[r["world_id"]] for r in krows) / n,
            "mean_savings": sum(savings) / n,
            "frac_saved": sum(1 for s in savings if s > 0) / n,
            "early_stops": sum(1 for r in krows if stop[r["world_id"]] < T_MAX),
        }
    return result
...
def main() -> int:
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l5-saturation-v1:evaluator",
        process_surface="benchmarking",
        task_id="BENEFIT-L5-SATURATION-V1",
        ...
        residual_before=("L5-benefit-unmeasured",),
        residual_after=() if evaluator_exit == 0 else ("L5-benefit-unmeasured",),
        retained_novelty=(),
    ...
    policy = DecisionPolicy("policy:benefit-l5-saturation-v1", epoch.epoch_id,
                            (("operator_cost", 1.0), ("residual_contraction", 0.0)))
    self_model = SelfModelSnapshot(
        self_model_hash=canonical_hash(("benefit-l5-saturation-v1", "self-model")),
        genome_hash=canonical_hash(
            ("rakl", "L5", "epistemic_saturation.audit_bounded_epistemic_saturation")),
        evaluation_epoch_id=epoch.epoch_id,
        episode_cutoff_hash=canonical_hash(("cutoff", arm_run_started_at)),
        context_signature=("BENEFIT-L5-SATURATION-V1",),
    )
    decision = shadow_decide(
        ...
        gates=gates, decision_id="decision:benefit-l5-saturation-v1-run-1")
    verdict = interpret_controller_for_runtime(decision)
    proposal = surface_governed_proposal(
        verdict, proposal_id="proposal:benefit-l5-saturation-v1-run-1", sign_off=None)
    ...
    write_json(os.path.join(RESULTS_DIR, "per_class_breakdown.json"), {
        "arm_A_budget_exhaustion_stopping": per_class_breakdown(corpus_rows, arm_a_path),
        "arm_B_saturation_stopping": per_class_breakdown(corpus_rows, arm_b_path),
    })
    print(json.dumps({
        "verdict": evaluator_report.get("verdict"),
        "completeness_b": evaluator_report.get("completeness_b"),
        "mean_savings_b": evaluator_report.get("mean_savings_b"),
