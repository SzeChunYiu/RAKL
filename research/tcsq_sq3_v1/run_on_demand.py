...
RESULT_FILE = HERE / "results" / "on_demand.json"
...
OBSERVATION_WINDOW = 1  # Minimum tasks before checking for first repeat
ACTIVATION_THRESHOLD = -1.0  # Trigger construction immediately (always activate)
...
CONTEXT_HASH = "ctx:tcsq-on-demand-known-world-v1"
...
ARMS = (
    ...
    "ON_DEMAND_FAMILY_CERTIFICATE_QUOTIENT",
)
...
# --------------------------------------------------------------------------
def run_on_demand_arm(
    tasks: list[dict[str, object]],
    label: str,
    *,
    probe_budget: int,
) -> dict[str, object]:
    """ON_DEMAND_FAMILY_CERTIFICATE_QUOTIENT: construct certificate at FIRST REPEAT; no observation window, no threshold.

    Phase 1: Check for first repeat in first min(1, N) tasks with simple structure-keyed
...
def run_replicate(seed: int, n_tasks: int, n_distinct: int, n_states: int, probe_budget: int) -> dict[str, object]:
    ...
    results = {
        ...
        "ON_DEMAND_FAMILY_CERTIFICATE_QUOTIENT": run_on_demand_arm(
            tasks, f"ondemand-{seed}-odq", probe_budget=probe_budget
        ),
    }
...
SUCCESSOR = "ON_DEMAND_FAMILY_CERTIFICATE_QUOTIENT"
...
def generate_results(
    *,
    ...
    return {
        "schema_version": "rakl.tcsq.on_demand.v1",
        ...
        "arms": {
            ...
            "ON_DEMAND_FAMILY_CERTIFICATE_QUOTIENT": "SUCCESSOR: adaptive construction; observe hit rate, activate only if threshold exceeded",
        },
