...
RESULT_FILE = HERE / "results" / "sq3_successor.json"
...
CONTEXT_HASH = "ctx:tcsq-sq3-successor-known-world-v1"
...
def summarise_cell(
    ...
    hits = [int(row["arms"][SUCCESSOR]["cache_hits"]) for row in rows]  # type: ignore[index]
    cert_replays = [int(row["arms"][SUCCESSOR]["certificate_replays"]) for row in rows]  # type: ignore[index]
    full_resolves = [int(row["arms"][SUCCESSOR]["full_resolve_verifications"]) for row in rows]  # type: ignore[index]
    invalid_rej = [int(row["arms"][SUCCESSOR]["invalid_certificate_rejections"]) for row in rows]  # type: ignore[index]
...
def generate_results(
    *,
    ...
    return {
        "schema_version": "rakl.tcsq.sq3_successor.family_certificate.v1",
        ...
        "historical_negative_reference": "research/tcsq_sq3_v1/results/sq3.json (KEEP_PROPOSAL_ONLY, net mean=-4074.34)",
        "mechanisms": {
            "A_per_family_validation": "erasure ledger validated once per coordinate-schema family via one assured_materialize_validated_quotient call; per-instance ledger cost amortises to 0",
            "B_certificate_verification": "retained witness plans certify over-budget/guard-refused negatives; reused refusals replayed (plan-length ops) not full-re-solved; genuinely-unreachable negatives still full-re-solved",
            "new_dimension_solve_cost_scale": "certificate checking is O(plan length), solving is O(states x actions); scaling the state space exposes the reuse-amortisation crossover",
        },
        "coordinate_schema": {
        ...
        "arms": {
            ...
            "FAMILY_CERTIFICATE_QUOTIENT": "SUCCESSOR: per-family validation + certificate verification with retained witnesses",
        },
