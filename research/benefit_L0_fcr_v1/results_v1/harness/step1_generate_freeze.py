"""Step 1 — generate the known-answer corpus and freeze it into the RSHEA
...
- audit_sample.json         40 seed-sampled pairs: surface texts + gold ONLY
...
from common import (  # noqa: E402
    ...
    sha256_file, utc_now_iso, write_json,
)
...
ARM_INPUT_FIELDS = (
    "pair_id", "facet_left", "facet_right", "value_left", "value_right",
    "surface_text_left", "surface_text_right", "context_left", "context_right",
)
...
def main() -> int:
    ...
    stripped = {
        ...
        "pairs": [{k: row[k] for k in ARM_INPUT_FIELDS} for row in corpus["pairs"]],
    }
    stripped_sha = write_json(stripped_path, stripped)

    rng_audit = random.Random(REGISTERED_SEED)
    by_id = {row["pair_id"]: row for row in corpus["pairs"]}
    audit_ids = rng_audit.sample(sorted(by_id), 40)
    audit_sample = {
        "note": ("auditor-facing sample: ONLY the two rendered claim texts and the "
                 "gold label; no machine fields, no arm outputs exist yet"),
        "sample_seed": REGISTERED_SEED,
        "pairs": [
            {
                "pair_id": pid,
                "surface_text_left": by_id[pid]["surface_text_left"],
                "surface_text_right": by_id[pid]["surface_text_right"],
                "gold_label": by_id[pid]["gold_label"],
            }
            for pid in audit_ids
        ],
    }
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l0-fcr-v1:corpus-freeze",
        ...
        residual_after=("L0-benefit-unmeasured",),
        retained_novelty=(),
