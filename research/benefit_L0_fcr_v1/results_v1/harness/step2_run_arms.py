...
Refuses to run if the corpus freeze manifest is missing (chronology guard) or if
the frozen corpus hash changed since freeze.
"""
...
def main() -> int:
    ...
    if str(manifest["label_minted_at"]) >= arm_run_started_at:
        print("CANNOT_CHECK: labels not strictly before arm start", file=sys.stderr)
        return 3
    ...
    for arm in ("A", "B"):
        ...
        telemetry = ProcessTelemetry(
            invocation_id=f"benefit-l0-fcr-v1:arm-{arm}",
            process_surface="contradiction_diagnosis",
        ...
        print(f"arm {arm}: declared={arm_out['n_declared_contradiction']} "
              f"time={arm_out['wall_clock_seconds']:.3f}s rss={arm_out['peak_rss_bytes']}")
