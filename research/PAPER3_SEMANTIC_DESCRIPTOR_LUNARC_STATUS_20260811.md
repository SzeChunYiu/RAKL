...
implementation, BGE revision and six required assets. It binds the shared CPU
runtime provenance and requires its exact tree identity to be created by the
allocated stage receipt; no pre-freeze tree digest is asserted. It also binds
every submission, batch, execution and harvest artifact. Native execution is
two separate LUNARC `lu48` allocations:

1. download the exact public model revision, recompute every size and SHA-256,
   atomically promote the snapshot only on an exact match, and attest the exact
   shared-runtime tree from inside that allocation;
...
before and after inference. The pre-inference runtime digest must equal the
allocated stage attestation; version strings alone create no authority.

Issue 43 was reported with zero external annotations, adjudications and
evaluated results at `2026-08-11T04:48:47Z`; the payload-free observation is
frozen as preflight evidence. It is not sufficient to promote a descriptor run
later. Descriptor harvest requires a fresh payload-free zero-label observation
made after the descriptor, or a first-label cutoff proving that the descriptor
...
harvest the model-stage/runtime-attestation job, and only then submit the
descriptor job with a current zero-label observation. Harvest it only after the
