...
- audit_sample.json         40 seed-sampled claims: surface text + gold ONLY
...
ARM_INPUT_FIELDS = (
    "claim_id", "claim", "proposal", "registry", "bindings",
    "frozen_before_promotion", "missing_obligations", "surface_text",
)
...
def main() -> int:
    ...
    errors = class_invariant_checks(corpus["claims"])
    if errors:
        for e in errors[:20]:
    ...
    stripped = {
        ...
        "claims": [{k: row[k] for k in ARM_INPUT_FIELDS} for row in corpus["claims"]],
    }
    stripped_sha = write_json(stripped_path, stripped)

    rng_audit = random.Random(REGISTERED_SEED)
    by_id = {row["claim_id"]: row for row in corpus["claims"]}
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l3-authority-v1:corpus-freeze",
        ...
        residual_after=("L3-benefit-unmeasured",),
        retained_novelty=(),
