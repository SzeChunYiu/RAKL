...
Also computes the per-class ALR/VUA breakdown with arm B refusal sub-verdicts
...
def per_class_breakdown(corpus_rows, arm_out_path):
    ...
    decl = {d["claim_id"]: d for d in rows}
    result = {}
    for klass in CLASSES:
        krows = [r for r in corpus_rows if r["class"] == klass]
        n = len(krows)
        leaks = sum(1 for r in krows
                    if decl[r["claim_id"]]["declaration"] == "UPGRADE"
                    and r["gold_label"] == "UNSUPPORTED")
        gold_pos = [r for r in krows if r["gold_label"] == "SUPPORTED"]
        vu = sum(1 for r in gold_pos
                 if decl[r["claim_id"]]["declaration"] == "UPGRADE")
        subs: dict[str, int] = {}
        for r in krows:
            sub = decl[r["claim_id"]].get("sub_verdict", decl[r["claim_id"]]["declaration"])
            subs[sub] = subs.get(sub, 0) + 1
        result[klass] = {
            "n": n,
            "unsupported_upgrades": leaks,
            "alr_within_class": leaks / n,
            "gold_supported": len(gold_pos),
            "valid_upgrades_accepted": vu,
            "vua_within_class": (vu / len(gold_pos)) if gold_pos else None,
            "sub_verdicts": subs,
        }
    return result
...
def main() -> int:
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l3-authority-v1:evaluator",
        process_surface="benchmarking",
        task_id="BENEFIT-L3-AUTHORITY-V1",
        ...
        residual_before=("L3-benefit-unmeasured",),
        residual_after=() if evaluator_exit == 0 else ("L3-benefit-unmeasured",),
        retained_novelty=(),
    ...
    policy = DecisionPolicy("policy:benefit-l3-authority-v1", epoch.epoch_id,
                            (("operator_cost", 1.0), ("residual_contraction", 0.0)))
    self_model = SelfModelSnapshot(
        self_model_hash=canonical_hash(("benefit-l3-authority-v1", "self-model")),
        genome_hash=canonical_hash(
            ("rakl", "L3", "evidence_binding_certificate.evaluate_evidence_binding_for_promotion")),
        evaluation_epoch_id=epoch.epoch_id,
        episode_cutoff_hash=canonical_hash(("cutoff", arm_run_started_at)),
        context_signature=("BENEFIT-L3-AUTHORITY-V1",),
    )
    decision = shadow_decide(
        ...
        gates=gates, decision_id="decision:benefit-l3-authority-v1-run-1")
    verdict = interpret_controller_for_runtime(decision)
    proposal = surface_governed_proposal(
        verdict, proposal_id="proposal:benefit-l3-authority-v1-run-1", sign_off=None)
    ...
    with open(corpus_path, "r", encoding="utf-8") as handle:
        corpus_rows = json.load(handle)["claims"]
    ...
    write_json(os.path.join(RESULTS_DIR, "per_class_breakdown.json"), {
        "arm_A_ungoverned_upgrade": per_class_breakdown(corpus_rows, arm_a_path),
        "arm_B_certificate_gated_upgrade": per_class_breakdown(corpus_rows, arm_b_path),
    })
    print(json.dumps({
        "verdict": evaluator_report.get("verdict"),
        "delta_alr": evaluator_report.get("delta_alr"),
