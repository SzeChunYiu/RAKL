...
Also computes the per-class UCR/VCA breakdown with arm B refusal sub-verdicts
...
def per_class_breakdown(corpus_rows, arm_out_path):
    ...
    decl = {d["chain_id"]: d for d in rows}
    result = {}
    for klass in CLASSES:
        krows = [r for r in corpus_rows if r["class"] == klass]
        n = len(krows)
        uc = sum(1 for r in krows
                 if decl[r["chain_id"]]["declaration"] == "COMPOSED"
                 and r["gold_label"] == "UNSUPPORTED")
        gold_pos = [r for r in krows if r["gold_label"] == "SUPPORTED"]
        vc = sum(1 for r in gold_pos
                 if decl[r["chain_id"]]["declaration"] == "COMPOSED")
        subs: dict[str, int] = {}
        for r in krows:
            sub = decl[r["chain_id"]].get("sub_verdict", "COMPOSED")
            subs[sub] = subs.get(sub, 0) + 1
        result[klass] = {
            "n": n,
            "unsupported_compositions": uc,
            "ucr_within_class": uc / n,
            "gold_supported": len(gold_pos),
            "valid_compositions_accepted": vc,
            "vca_within_class": (vc / len(gold_pos)) if gold_pos else None,
            "sub_verdicts": subs,
        }
    return result
...
def main() -> int:
    ...
    telemetry = ProcessTelemetry(
        invocation_id="benefit-l1-composition-v1:evaluator",
        ...
        residual_after=() if evaluator_exit == 0 else ("L1-benefit-unmeasured",),
        retained_novelty=(),
    ...
    policy = DecisionPolicy("policy:benefit-l1-composition-v1", epoch.epoch_id,
                            (("operator_cost", 1.0), ("residual_contraction", 0.0)))
    self_model = SelfModelSnapshot(
        self_model_hash=canonical_hash(("benefit-l1-composition-v1", "self-model")),
        genome_hash=canonical_hash(("rakl", "L1", "bridge_composition.evaluate_bridge_path")),
        evaluation_epoch_id=epoch.epoch_id,
        episode_cutoff_hash=canonical_hash(("cutoff", arm_run_started_at)),
        context_signature=("BENEFIT-L1-COMPOSITION-V1",),
    )
    decision = shadow_decide(
        ...
        gates=gates, decision_id="decision:benefit-l1-composition-v1-run-1")
    verdict = interpret_controller_for_runtime(decision)
    proposal = surface_governed_proposal(
        verdict, proposal_id="proposal:benefit-l1-composition-v1-run-1", sign_off=None)
    ...
    with open(corpus_path, "r", encoding="utf-8") as handle:
        corpus_rows = json.load(handle)["chains"]
    ...
    write_json(os.path.join(RESULTS_DIR, "per_class_breakdown.json"), {
        "arm_A_untyped_chaining": per_class_breakdown(corpus_rows, arm_a_path),
        "arm_B_typed_transition_algebra": per_class_breakdown(corpus_rows, arm_b_path),
    })
    print(json.dumps({
        "verdict": evaluator_report.get("verdict"),
        "delta_ucr": evaluator_report.get("delta_ucr"),
