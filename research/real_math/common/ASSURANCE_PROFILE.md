...
## Pre-candidate discovery gate

For every **new** mathematical candidate created under this profile, the active atomic obstruction must first have a frozen `MathContextFiber` satisfying `schemas/math-context-fiber.schema.json` and `audit_math_context_fiber`.

The context packet must precede candidate generation and contain the atom's structural coordinates, equivalent formulations, solved/near-solved analogous contexts, method-transfer assumptions, shared structure, explicit disanalogies, repair questions, source anchors and content hash.

A paper list, generic literature summary, or after-the-fact explanation does not pass this gate. If `plan_math_research(..., context_fiber=...)` reports `candidate_generation_allowed: false`, only `pre_candidate_actions` may be executed.

### No retroactive grandfathering

Candidates and negative checkpoints created before this gate existed remain valid historical research artifacts at their already recorded authority levels. They must **not** be relabeled as strict context-first discoveries by backfilling a packet after the fact.

After this gate is merged, any materially new successor candidate must begin from a freshly frozen context fiber. Existing residuals may be reused as the atomic obstruction, but the analogue/method-transfer analysis and chronology must be newly frozen before the successor candidate is generated.

## Non-compensatory gates

1. **Discovery-process compliance**. If the work is claimed as strict RAKL-mediated discovery, the pre-candidate context chronology must pass. This gate does not substitute for mathematical truth.
...
- If the root candidate is generated after adoption of the context gate, its exact parent obstruction has a pre-candidate context packet with valid chronology.
